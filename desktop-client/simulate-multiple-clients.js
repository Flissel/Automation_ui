...
const connections = simulatedClients.map((client, clientIndex) => {
  ...
  ws.on('open', () => {
    ...
    const frameInterval = setInterval(() => {
      ...
      if (frameCount % 50 === 0) {
        console.log(`[${clientIndex + 1}] 📺 ${client.hostname}: ${frameCount} frames sent`);
      }
    }, 100); // 10 fps
    ...
    try {
      ...
      switch (message.type) {
        case 'handshake_ack':
          ...
          break;
        case 'start_capture':
          console.log(`[${clientIndex + 1}] 📬 Received start_capture for ${message.monitorId}`);
          break;
        case 'stop_capture':
          console.log(`[${clientIndex + 1}] 📬 Received stop_capture for ${message.monitorId}`);
          break;
      }
    } catch (error) {
      // Ignore parse errors
  ...
  ws.on('error', (error) => {
    console.error(`[${clientIndex + 1}] ❌ Error (${client.hostname}):`, error.message);
  });
