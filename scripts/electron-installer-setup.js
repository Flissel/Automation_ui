...
async function installPython(logger) {
    if (os.platform() !== 'win32') {
        logger.error('Automatische Python-Installation nur unter Windows unterstützt');
        return false;
    }
    ...
    try {
        ...
        execSync(`curl -L -o "${installerPath}" "${pythonUrl}"`, { stdio: 'inherit' });

        logger.info('Installiere Python...');
        execSync(`"${installerPath}" /quiet InstallAllUsers=0 PrependPath=1 Include_pip=1`, { 
            stdio: 'inherit' 
        });
...
function installPythonDependencies(logger, installPath) {
    ...
    try {
        ...
        if (fs.existsSync(requirementsFile)) {
            execSync(`python -m pip install -r "${requirementsFile}"`, { stdio: 'pipe' });
        } else {
...
function setupWindowsAutostart(logger, installPath) {
    ...
    const desktopClientPath = path.join(installPath, CONFIG.desktopClientFolder);
    const pythonScript = path.join(desktopClientPath, 'dual_screen_capture_client.py');
...
function startDesktopClient(logger, installPath) {
    ...
    try {
        ...
        const child = spawn('python', [
            pythonScript,
            '--fps', CONFIG.streamSettings.fps.toString(),
            '--quality', CONFIG.streamSettings.quality.toString()
        ], {
...
async function main() {
    ...
    logger.info('==========================================');
    logger.info(success ? 'Installation erfolgreich!' : 'Installation mit Fehlern beendet');
    logger.info('==========================================');
