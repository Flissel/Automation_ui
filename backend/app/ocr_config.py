...
class OCRServiceConfig(BaseSettings):
    ...
    max_concurrent_requests: int = Field(
        ...
        description="Maximum concurrent OCR requests"
    )
    ...
    image_quality: int = Field(
        default=95, ge=50, le=100,
        description="Image quality for processing (1-100)"
    )
    ...
    log_level: str = Field(
        default="INFO",
        description="Logging level for OCR service"
    )
    ...
    def validate_default_language(cls, v, info):
        ...
        supported = info.data.get('supported_languages', ["eng", "deu", "fra", "spa", "ita", "rus", "chi_sim", "jpn"])
...
class OCRMonitoringConfig(BaseSettings):
    ...
    enabled: bool = Field(
        default=True,
        description="Enable OCR monitoring"
    )
    
    # OCR settings for monitoring
    ocr_language: str = Field(
        default="eng+deu",
        description="OCR language for monitoring"
    )
    ...
    max_monitoring_regions: int = Field(
        ...
        description="Maximum number of monitoring regions"
    )
