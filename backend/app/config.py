...
class Settings(BaseSettings):
    ...
    file_watcher_patterns: List[str] = Field(default=["*.txt", "*.log", "*.json"], env="FILE_WATCHER_PATTERNS")
    file_watcher_debounce_interval: float = Field(default=0.5, env="FILE_WATCHER_DEBOUNCE_INTERVAL")
    ...
    websocket_timeout: int = Field(default=300, env="WEBSOCKET_TIMEOUT")  # Increased from 60 to 300 seconds
    max_websocket_connections: int = Field(default=100, env="MAX_WEBSOCKET_CONNECTIONS")
    websocket_message_buffer_size: int = Field(default=1000, env="WEBSOCKET_MESSAGE_BUFFER_SIZE")
    websocket_ping_interval: int = Field(default=60, env="WEBSOCKET_PING_INTERVAL")  # Increased from 30 to 60 seconds
    websocket_ping_timeout: int = Field(default=30, env="WEBSOCKET_PING_TIMEOUT")  # Increased from 10 to 30 seconds
    websocket_close_timeout: int = Field(default=15, env="WEBSOCKET_CLOSE_TIMEOUT")  # Increased from 5 to 15 seconds
    websocket_max_consecutive_errors: int = Field(default=10, env="WEBSOCKET_MAX_CONSECUTIVE_ERRORS")  # Increased from 3 to 10
    websocket_connection_stability_delay: float = Field(default=0.5, env="WEBSOCKET_CONNECTION_STABILITY_DELAY")  # Increased from 0.1 to 0.5
    websocket_enable_compression: bool = Field(default=True, env="WEBSOCKET_ENABLE_COMPRESSION")
    websocket_max_message_size: int = Field(default=2097152, env="WEBSOCKET_MAX_MESSAGE_SIZE")  # Increased from 1MB to 2MB
    ...
    ocr_monitoring_similarity_threshold: float = Field(default=0.85, env="OCR_MONITORING_SIMILARITY_THRESHOLD")
    ...
    proxmox_default_node: str = Field(default="ubuntu-2gb-nbg1-1", env="PROXMOX_DEFAULT_NODE")
    ...
    default_api_keys: List[str] = Field(
        default=["trae-default-api-key-2024"],
    ...
    screenshots_dir: Path = Field(default=Path("desktop_screenshots"), env="SCREENSHOTS_DIR")
        ...
        try:
            ...
            if isinstance(v, str):
                if v.strip():
                    ...
                    return valid_origins if valid_origins else [
                        "http://localhost:3000",
                        "http://localhost:3005",
                        "http://localhost:5174",
                        "http://localhost:8080"
    ...
    def validate_ocr_monitoring_interval(cls, v):
        ...
        if not 1 <= v <= 3600:
            raise ValueError("OCR monitoring interval must be between 1 and 3600 seconds")
        return v
    ...
    def validate_ocr_monitoring_similarity_threshold(cls, v):
        ...
        if not 0.0 <= v <= 1.0:
            raise ValueError("OCR monitoring similarity threshold must be between 0.0 and 1.0")
        return v
