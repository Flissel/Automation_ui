...
class SnapshotRequest(BaseModel):
    ...
    region: Optional[Dict[str, int]] = Field(None, description="Optional region {x, y, width, height}")
...
class OCRZoneConfig(BaseModel):
    ...
    preprocessing: Optional[Dict[str, Any]] = Field(None, description="OCR preprocessing options")
...
class SnapshotTemplate(BaseModel):
    ...
    ocr_zones: List[OCRZoneConfig] = Field(default_factory=list, description="OCR zone configurations")
    click_actions: List[ClickActionConfig] = Field(default_factory=list, description="Click action configurations")
...
class ClickExecutionRequest(BaseModel):
    ...
    actions: List[ClickActionConfig] = Field(..., description="Click actions to execute")
    sequential: bool = Field(True, description="Execute actions sequentially")
    delay_between_actions: int = Field(500, ge=0, description="Delay between actions (ms)")
...
class OCRResult(BaseModel):
    ...
    bounding_box: Optional[Dict[str, int]] = Field(None, description="Text bounding box")
