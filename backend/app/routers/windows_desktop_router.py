...
# Create router
router = APIRouter(
    tags=["windows-desktop"], responses={404: {"description": "Not found"}}
)
...
class WindowsDesktopStreamingRequest(BaseModel):
    ...
    connection_name: str = Field(
        default="host_system", description="SSH connection name"
    )
    fps: Optional[int] = Field(default=8, ge=1, le=30, description="Frames per second")
    scale_factor: Optional[float] = Field(
        default=0.8, ge=0.1, le=2.0, description="Scale factor"
    )
    quality: Optional[int] = Field(
        default=85, ge=10, le=100, description="Image quality"
    )
...
class DesktopSwitchRequest(BaseModel):
    ...
    force: bool = Field(
        default=False, description="Force switch even if target unavailable"
    )
...
class SSHConnectionTestRequest(BaseModel):
    ...
    test_screenshot: bool = Field(
        default=True, description="Test screenshot capability"
    )
...
async def capture_windows_screenshot():
    ...
    try:
        ...
        if not windows_service.current_connection:
            ...
            if not connected:
                raise HTTPException(
                    status_code=400, detail="No active SSH connection to Windows host"
                )
        ...
        if image_data and resolution:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to capture Windows desktop screenshot"
            )
...
async def test_ssh_connection(request: SSHConnectionTestRequest):
    ...
    try:
        ...
        # Test connection
        connected = await windows_service.connect_to_windows_host(
            ...
            connections.append(
                {
                    ...
                    "last_connected": (
                        status.last_connected.isoformat()
                        if status and status.last_connected
                        else None
                    ),
                }
            )
...
async def get_hybrid_desktop_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get hybrid status: {str(e)}"
        )
