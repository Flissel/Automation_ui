...
async def start_ocr_monitoring(request: OCRMonitoringStartRequest):
    ...
    try:
        ...
        if not monitoring_service:
            raise HTTPException(
                status_code=503, detail="OCR monitoring service not available"
            )
...
async def stop_ocr_monitoring(request: Request):
    ...
    try:
        ...
        return JSONResponse(
            content={"success": True, "message": "OCR monitoring stopped successfully"}
        )
...
async def get_ocr_monitoring_health(request: Request):
    ...
    try:
        ...
        health_status = {
            ...
            "dependencies": {
                "ocr_service": (
                    monitoring_service.ocr_service.is_healthy()
                    if monitoring_service.ocr_service
                    else False
                ),
                "live_desktop_service": (
                    monitoring_service.live_desktop_service.is_healthy()
                    if monitoring_service.live_desktop_service
                    else False
                ),
