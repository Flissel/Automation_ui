...
async def health_check(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Health check system unavailable"
        )
...
async def detailed_health_check(request: Request):
    ...
    try:
        ...
        all_healthy = all(info.get("healthy", False) for info in detailed_status.values())
        
        health_data = {
            "status": "healthy" if all_healthy else "degraded",
            "services": detailed_status,
            "service_count": len(detailed_status),
            "healthy_count": sum(1 for info in detailed_status.values() if info.get("healthy", False)),
            "timestamp": datetime.utcnow().isoformat(),
            "version": "1.0.0"
        }
