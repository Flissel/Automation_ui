...
async def list_virtual_desktops(request: Request):
    ...
    try:
        ...
        if not desktop_service:
            raise HTTPException(status_code=503, detail="Desktop automation service not available")
        ...
        for session_id, session_data in active_sessions.items():
            desktop = VirtualDesktop(
                ...
                metadata={
                    ...
                    "click_sequence_length": len(session_data.get("click_sequence", []))
                }
            )
...
async def connect_to_desktop(request: Request, desktop_data: Dict[str, Any]):
    ...
    try:
        ...
        if not session_info.get("success", False):
            raise HTTPException(status_code=404, detail=f"Desktop {desktop_id} not found")
...
async def execute_workflow(request: Request, workflow_data: Dict[str, Any]):
    ...
    try:
        ...
        from ..services import get_websocket_manager, get_click_automation_service, get_desktop_automation_service, get_ocr_service
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail=f"Graph execution service not available: {e}")
        ...
        logger.info(f"V1 API: Started workflow execution {execution_id} for workflow {workflow_id}")
...
async def execute_automation_actions(request: Request, actions_data: Dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"V1 API: Executed {successful_actions}/{len(actions)} automation actions")
...
@log_api_request(logger)
async def take_automation_screenshot(request: Request, screenshot_data: Dict[str, Any] = None):
...
async def get_health_status(request: Request):
    ...
    try:
        ...
        logger.info(f"V1 API: Health check completed - status: {health_status['status']}")
