...
class LiveDesktopService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if test_screenshot:
                ...
                logger.warning("⚠️ Desktop screenshot test failed, but service will continue")
    ...
    async def start_streaming(self, client_id: str, client_info: Dict[str, Any], monitor_id: str = "monitor_0") -> bool:
        ...
        """
        try:
            logger.info(f"🎬 Starting desktop streaming for client: {client_id} on monitor: {monitor_id}")
            ...
            logger.info(f"✅ Desktop streaming started for client: {client_id} on {monitor_id}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Failed to start streaming for client {client_id} on {monitor_id}: {e}")
            return False
    ...
    async def stop_streaming(self, client_id: str, monitor_id: Optional[str] = None) -> bool:
        ...
        """
        try:
            logger.info(f"🛑 Stopping desktop streaming for client: {client_id}{' on ' + monitor_id if monitor_id else ''}")
            ...
            logger.info(f"✅ Desktop streaming stopped for client: {client_id}{' on ' + monitor_id if monitor_id else ''}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Failed to stop streaming for client {client_id}{' on ' + monitor_id if monitor_id else ''}: {e}")
            return False
    ...
    def remove_frame_callback(self, callback: Callable[[bytes], None]):
        ...
        if callback in self.frame_callbacks:
            ...
            logger.debug(f"📡 Frame callback removed, total: {len(self.frame_callbacks)}")
