...
class ServiceManager:
    ...
    def get_service(self, service_name: str) -> Optional[Any]:
        ...
        if not self.initialized:
            logger.warning(f"⚠️ Service manager not initialized, cannot get service '{service_name}'")
            return None
            
        service = self.services.get(service_name)
        if service is None:
            logger.warning(f"⚠️ Service '{service_name}' not found. Available services: {list(self.services.keys())}")
