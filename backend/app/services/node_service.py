...
from ..schemas.node_schemas import (
    Node, NodeGraph, NodeConnection, NodeTemplate, NodeLibrary,
    NodeType, NodeStatus, ConnectionType, DataType,
    NodeExecutionRequest, NodeExecutionResult, GraphExecutionResult,
    NodeSearchRequest, NodeUpdateRequest, ConnectionCreateRequest,
    GraphCreateRequest, GraphUpdateRequest, GraphValidationResult,
    NodeValidationError, NodeStatistics, NodePosition, NodeSize,
    NodePort, NodeData, NodeConfiguration, NodeMetadata
)
...
class NodeService:
    ...
    async def _initialize_builtin_templates(self):
        ...
        builtin_templates = [
            ...
            NodeTemplate(
                ...
                input_ports=[
                    NodePort(id="data", name="Data", data_type=DataType.ANY, required=True)
                ],
            ...
            NodeTemplate(
                ...
                input_ports=[
                    NodePort(id="input", name="Input", data_type=DataType.STRING, required=True),
                    NodePort(id="operation", name="Operation", data_type=DataType.STRING, default_value="uppercase")
                ],
                output_ports=[
            ...
            NodeTemplate(
                ...
                input_ports=[
                    NodePort(id="condition", name="Condition", data_type=DataType.BOOLEAN, required=True)
                ],
                output_ports=[
    ...
    async def create_graph(self, request: GraphCreateRequest) -> NodeGraph:
        ...
        await websocket_manager.publish_to_topic("graph_updates", {
            "type": "graph_created",
            "graph_id": graph_id,
            "graph": graph.dict()
        })
    ...
    async def update_graph(self, graph_id: str, request: GraphUpdateRequest) -> NodeGraph:
        """Update a graph"""
    ...
    # Node Management
    async def create_node(self, graph_id: str, template_id: str, position: NodePosition) -> Node:
        ...
        # Notify WebSocket clients
        await websocket_manager.publish_to_topic("node_updates", {
            "type": "node_created",
            "graph_id": graph_id,
            "node": node.dict()
        })
    ...
    async def update_node(self, graph_id: str, node_id: str, request: NodeUpdateRequest) -> Node:
        """Update a node"""
    ...
    async def create_connection(self, graph_id: str, request: ConnectionCreateRequest) -> NodeConnection:
        ...
        source_port = next((p for p in source_node.output_ports if p.id == request.source_port_id), None)
        target_port = next((p for p in target_node.input_ports if p.id == request.target_port_id), None)
        
        if not source_port or not target_port:
            raise HTTPException(status_code=404, detail="Source or target port not found")
        
        # Check for existing connection to target port (if not multiple)
        if not target_port.multiple:
            existing = next((c for c in graph.connections 
                           if c.target_node_id == request.target_node_id 
                           and c.target_port_id == request.target_port_id), None)
            if existing:
                raise HTTPException(status_code=400, detail="Target port already connected")
        ...
        # Notify WebSocket clients
        await websocket_manager.publish_to_topic("connection_updates", {
    ...
    # Execution
    async def execute_node(self, graph_id: str, node_id: str, inputs: Dict[str, Any]) -> NodeExecutionResult:
        ...
        try:
            ...
            # Notify WebSocket clients
            await websocket_manager.publish_to_topic("execution_updates", {
    ...
    async def _execute_node_logic(self, node: Node, inputs: Dict[str, Any]) -> Dict[str, Any]:
