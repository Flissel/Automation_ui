...
async def capture_current_frame(
    quality: int = 50, viewport: dict = None
) -> Optional[str]:
...
class VideoAgent:
    ...
    def __init__(self, guardian_mode: bool = False, monitor_mode: bool = False):
        ...
        if monitor_mode:
            logger.info(
                "[VideoAgent] Monitor Mode ENABLED - realtime monitoring active"
            )
    ...
    async def analyze_frame(
        self, frame_base64: str, context: Dict[str, Any], conversation_id: str
    ) -> Dict[str, Any]:
        """
        ...
        if not self._enabled:
            return {
                "error": "Video agent disabled (no API key)",
        ...
        step_record = {
            ...
            "result_success": (
                context.get("result", {}).get("success")
    ...
    async def analyze_and_guard(
        ...
        result = {"analysis": analysis, "corrections": [], "final_status": "unknown"}

        # 2. Guardian Mode disabled? Return immediately
        if not self.guardian_mode or not self._tool_executor:
            result["final_status"] = (
                "verified" if analysis.get("action_verified") else "unverified"
            )
            return result
        ...
        for attempt in range(max_retries):
            ...
            logger.info(
                f"[Guardian] Attempt {attempt + 1}/{max_retries}: {correction_action['tool']}({correction_action['args']})"
            )
            ...
            # Re-capture frame
            new_frame = await capture_current_frame(
                quality=50, viewport=context.get("viewport")
            )
            if not new_frame:
            ...
            if analysis.get("action_verified") and analysis.get("confidence", 0) > 0.7:
                ...
                logger.info(
                    f"[Guardian] Correction successful after {attempt + 1} attempt(s)"
                )
                return result
    ...
    def _plan_correction(self, analysis: Dict, context: Dict) -> Optional[Dict]:
        ...
        if "not visible" in screen_state or "off screen" in screen_state:
            ...
            return {"tool": "action_scroll", "args": {"direction": "down", "amount": 3}}
        ...
        if "dialog" in screen_state or "popup" in screen_state:
            ...
            return {"tool": "action_press", "args": {"key": "escape"}}
    ...
    async def _execute_correction(self, correction: Dict) -> Dict:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "[Guardian] MCP handlers not available - cannot execute corrections"
                )
                return {"success": False, "error": "MCP handlers not available"}

            # Map tool names to handlers
            if tool == "mouse_move":
                result = await action_tools.handle_mouse_move(
    ...
    async def start_monitor(self, conversation_id: str):
        ...
        try:
            ...
            StreamFrameCache.add_listener(
                self._monitor_callback_id, self._on_frame_update
            )
            ...
            logger.error(
                "[VideoAgent] StreamFrameCache not available - cannot start monitor"
            )
    ...
    async def _monitor_loop(self, conversation_id: str):
        ...
        try:
            while True:
                ...
                # Get fresh frame
                try:
                    from moire_agents.stream_frame_cache import \
                        StreamFrameCache

                    frame = StreamFrameCache.get_fresh_frame(
                        monitor_id=0, max_age_ms=1000
                    )
                except ImportError:
                ...
                try:
                    analysis = await self.analyze_frame(
                        frame.data, context, conversation_id
                    )
                    ...
                    if event_type == "dialog":
                        ...
                        # Alert user
                        logger.warning(
                            f"[Monitor] Unexpected change: {analysis.get('screen_state')}"
                        )
                    ...
                    error_count += 1
                    logger.error(
                        f"[Monitor] Analysis failed ({error_count}/{max_errors}): {e}"
                    )
    ...
    def save_training_data(self, conversation_id: str, task_summary: str) -> str:
        ...
        filename = (
            f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{conversation_id[:8]}.json"
        )
    ...
    def get_session_stats(self, conversation_id: str) -> Dict[str, Any]:
        ...
        return {"steps_analyzed": len(steps), **self._generate_summary(steps)}
...
_GUARDIAN_MODE = (
    _os_config.getenv("VIDEO_AGENT_GUARDIAN_MODE", "false").lower() == "true"
)
