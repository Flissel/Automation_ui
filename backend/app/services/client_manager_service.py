...
class ClientManagerService:
    ...
    def _find_client_script(self) -> Path:
        ...
        logger.warning(
            f"[ClientManager] Script nicht gefunden, verwende Default: {default_path}"
        )
        return default_path
    ...
    async def start_client(
        self, auto_restart: bool = True, server_url: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not self._script_path.exists():
                raise FileNotFoundError(
                    f"Client-Script nicht gefunden: {self._script_path}"
                )
            ...
            logger.info(
                f"[ClientManager] Client gestartet mit PID: {self._process.pid}"
            )
            ...
            if self._process:
                if force:
                    ...
                    try:
                        ...
                        logger.warning(
                            "[ClientManager] Timeout beim Stoppen, force kill..."
                        )
                        self._process.kill()
    ...
    def receive_heartbeat(
        ...
        with self._lock:
            ...
            if self._status != ClientStatus.RUNNING and self._process:
                self._status = ClientStatus.RUNNING
                logger.info(
                    "[ClientManager] Client sendet wieder Heartbeats - Status korrigiert"
                )
                ...
                with self._lock:
                    ...
                    if self._process:
                        ...
                        if poll_result is not None:
                            logger.warning(
                                f"[ClientManager] Prozess unerwartet beendet (Code: {poll_result})"
                            )
                    ...
                    if self._last_heartbeat:
                        ...
                        if time_since_heartbeat > self.HEARTBEAT_TIMEOUT_SECONDS:
                            logger.warning(
                                f"[ClientManager] Heartbeat-Timeout ({time_since_heartbeat:.1f}s)"
                            )
                            self._trigger_restart("Heartbeat Timeout")

                    # Reset Restart-Counter nach Cooldown
                    if (
                        datetime.now() - self._last_restart_reset
                    ).total_seconds() > self.RESTART_COOLDOWN_MINUTES * 60:
                        self._restart_count = 0
    ...
    def _trigger_restart(self, reason: str):
        ...
        if self._restart_count >= self.MAX_RESTART_ATTEMPTS:
            logger.error(
                f"[ClientManager] Max Restart-Versuche erreicht ({self.MAX_RESTART_ATTEMPTS})"
            )
            self._status = ClientStatus.ERROR
            return
        ...
        logger.info(
            f"[ClientManager] Auto-Restart #{self._restart_count} wegen: {reason}"
        )
