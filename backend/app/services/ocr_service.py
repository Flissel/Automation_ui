...
class OCREngineConfig(BaseModel):
    ...
    easyocr_config: Dict[str, Any] = Field(default_factory=lambda: {
        "gpu": False,
        "model_storage_directory": None,
        "download_enabled": True
    })
    paddleocr_config: Dict[str, Any] = Field(default_factory=lambda: {
        "use_angle_cls": True,
        "lang": "en",
        "use_gpu": False,
        "show_log": False
...
class OCRService:
    ...
    def _init_engines(self) -> None:
        ...
        if not self.available_engines:
            logger.error("No OCR engines available! Install pytesseract, easyocr, or paddleocr.")
    ...
    def _preprocess_image(self, image: Image.Image, preprocessing: Dict[str, Any]) -> Image.Image:
    ...
    def _extract_zone(self, image: Image.Image, zone_config: OCRZoneConfig) -> Image.Image:
    ...
    def _ocr_with_tesseract(self, image: Image.Image, zone_config: OCRZoneConfig) -> Tuple[str, float, List[BoundingBox]]:
    ...
    def _ocr_with_easyocr(self, image: Image.Image, zone_config: OCRZoneConfig) -> Tuple[str, float, List[BoundingBox]]:
    ...
    def _ocr_with_paddleocr(self, image: Image.Image, zone_config: OCRZoneConfig) -> Tuple[str, float, List[BoundingBox]]:
    ...
    async def process_zone(self, image_data: bytes, zone_config: OCRZoneConfig, return_image: bool = False) -> OCRZoneResult:
        ...
        try:
            ...
            # Preprocess image
            processed_image = self._preprocess_image(zone_image, zone_config.preprocessing)
            ...
            # Perform OCR
            if engine == "tesseract":
                text, confidence, bboxes = await asyncio.get_event_loop().run_in_executor(
                    self.executor, self._ocr_with_tesseract, processed_image, zone_config
                )
            elif engine == "easyocr":
                text, confidence, bboxes = await asyncio.get_event_loop().run_in_executor(
                ...
                text, confidence, bboxes = await asyncio.get_event_loop().run_in_executor(
                    self.executor, self._ocr_with_paddleocr, processed_image, zone_config
                )
            else:
            ...
            logger.debug(f"OCR completed for zone {zone_config.id}: '{text[:50]}...' (confidence: {confidence:.2f})")
            return result
    ...
    async def process_multiple_zones(self, image_data: bytes, zones: List[OCRZoneConfig], return_images: bool = False) -> List[OCRZoneResult]:
        ...
        tasks = [
            self.process_zone(image_data, zone, return_images)
            for zone in zones
        ]
        ...
        logger.info(f"OCR completed: {len(successful_results)}/{len(zones)} zones successful")
