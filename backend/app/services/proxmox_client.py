...
class ProxmoxClient:
    ...
    async def clone_vm(
        ...
        try:
            await self._make_request(
                "POST", f"/nodes/{node}/qemu/{source_vmid}/clone", clone_data
            )
            logger.info(
                f"Successfully cloned VM {source_vmid} to {target_vmid} ({name})"
            )
            return True
        except ProxmoxAPIError as e:
    ...
    async def get_vm_vnc_info(self, node: str, vmid: int) -> Dict[str, Any]:
        ...
        try:
            result = await self._make_request(
                "POST", f"/nodes/{node}/qemu/{vmid}/vncproxy"
            )
    ...
    async def get_vm_screenshot(self, node: str, vmid: int) -> bytes:
        ...
        try:
            ...
            result = await self._make_request(
                "GET", f"/nodes/{node}/qemu/{vmid}/screenshot"
            )
    ...
    async def wait_for_vm_status(
        self, node: str, vmid: int, target_status: str, timeout: int = 60
    ) -> bool:
        ...
        while True:
            try:
                ...
                if current_time - start_time > timeout:
                    logger.warning(
                        f"Timeout waiting for VM {vmid} to reach status {target_status}"
                    )
                    return False
