...
class ClickAutomationService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if not PYAUTOGUI_AVAILABLE:
                logger.error("❌ pyautogui not available - cannot initialize click automation")
                self.initialized = False
                self.healthy = False
                return
            ...
            logger.info(f"📺 Screen size detected: {screen_size.width}x{screen_size.height}")
    ...
    async def perform_click(
        ...
        try:
            ...
            logger.info(f"🖱️ Performing {click_type} {button} click at ({x}, {y}) with delay {delay}s")
            
            # Validate coordinates
            screen_size = self.get_screen_size()
            if screen_size:
                if x < 0 or x > screen_size["width"] or y < 0 or y > screen_size["height"]:
                    logger.warning(f"⚠️ Click coordinates ({x}, {y}) outside screen bounds {screen_size}")
