...
class ShellService:
    ...
    def check_command_security(self, command: str, shell_type: str, force: bool = False) -> Tuple[bool, str]:
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, command, re.IGNORECASE):
                logger.warning(f"Dangerous command detected: {command[:50]}... (pattern: {pattern})")
    ...
    def execute_command(self, command: str, shell_type: str, 
        ...
        # Security check
        is_safe, security_msg = self.check_command_security(command, shell_type, force_execution)
        if not is_safe:
    ...
    def create_session(self, shell_type: str, name: Optional[str] = None,
                      working_directory: Optional[str] = None,
                      environment: Optional[Dict[str, str]] = None,
    ...
    def get_environment_variables(self, session_id: Optional[str] = None) -> Dict[str, str]:
    ...
    def set_environment_variables(self, variables: Dict[str, str], 
        ...
        if session_id:
            ...
            logger.info(f"Updated environment for session {session_id}: {list(variables.keys())}")
        else:
    ...
    def change_directory(self, path: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if session_id:
            ...
            logger.info(f"Changed directory for session {session_id}: {old_path} -> {abs_path}")
        else:
    ...
    def manage_process(self, process_id: Optional[int] = None, 
                      process_name: Optional[str] = None,
                      action: str = "kill", force: bool = False) -> Dict[str, Any]:
        ...
        try:
            if action == "kill":
                ...
                result["message"] = f"Process {target_proc.pid} {'killed' if force else 'terminated'}"
