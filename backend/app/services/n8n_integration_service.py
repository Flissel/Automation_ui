...
try:
    from .event_broker_service import EventBrokerService, UnityAIEvent, EventType, AgentType
except ImportError:
...
class N8nIntegrationService:
    ...
    async def _setup_event_subscriptions(self):
        ...
        await self.event_broker.subscribe(
            EventType.DESKTOP_ACTION,
            self._handle_desktop_action_event
        )
        ...
        await self.event_broker.subscribe(
            ...
            self._handle_file_change_event
        )
        
        await self.event_broker.subscribe(
            EventType.QUALITY_GATE_FAILED,
            self._handle_quality_gate_event
        )
    ...
    async def _handle_desktop_action_event(self, event: UnityAIEvent):
        ...
        try:
            ...
            workflows = await self._find_workflows_by_trigger(TriggerType.DESKTOP_ACTION)
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.debug(f"n8n health check successful: {len(data)} active workflows")
    ...
    async def _update_active_executions(self, active_data: List[Dict]):
        ...
        for exec_id in completed:
            ...
            if self.event_broker:
                await self.event_broker.publish(UnityAIEvent(
        ...
        try:
            ...
            async with self.session.post(url, json=n8n_workflow) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Failed to create workflow: {response.status} - {error_text}")
                    
        except Exception as e:
            logger.error(f"Error creating workflow: {e}")
            raise
    
    async def _convert_to_n8n_format(self, workflow: WorkflowDefinition) -> Dict[str, Any]:
    ...
    async def _create_workflow_webhooks(self, workflow_id: str, workflow: WorkflowDefinition):
        ...
        for trigger in workflow.triggers:
            if trigger.get('type') == TriggerType.WEBHOOK.value:
                ...
                if webhook_id:
                    ...
                    logger.info(f"Mapped webhook {webhook_id} to workflow {workflow_id}")
    ...
    async def trigger_workflow(
        ...
        try:
            url = urljoin(self.config.base_url, f"/rest/workflows/{workflow_id}/execute")
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    execution = WorkflowExecution(
                        ...
                        finished_at=datetime.fromisoformat(data['stoppedAt'].replace('Z', '+00:00')) if data.get('stoppedAt') else None,
                        data=data.get('data', {}),
                    ...
                    logger.warning(f"Execution {execution_id} not found: {response.status}")
                    return None
        ...
        try:
            ...
            async with self.session.post(url) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Failed to cancel execution {execution_id}: {response.status}")
                    return False
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Workflow {workflow_id} not found: {response.status}")
                    return None
            ...
            async with self.session.delete(url) as response:
                if response.status == 200:
                    ...
                    self.webhook_mappings = {
                        k: v for k, v in self.webhook_mappings.items() 
                        if v != workflow_id
                    }
                    ...
                    logger.warning(f"Failed to delete workflow {workflow_id}: {response.status}")
                    return False
...
class UnityAIWorkflowTemplates:
    ...
    def file_watcher_workflow() -> Dict[str, Any]:
        ...
        return {
            ...
            'triggers': [
                {
                    'type': TriggerType.FILE_CHANGE.value,
                    'enabled': True
                }
            ],
            'settings': {
