...
class DesktopAutomationService(LoggerMixin):
    ...
    async def initialize(self):
        ...
        try:
            ...
            try:
                result = subprocess.run(['powershell', '-Command', 'Get-Process | Select-Object -First 1'], 
    ...
    async def create_desktop_session(self, session_config: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    async def execute_powershell_command(self, session_id: str, command: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_info(f"Executing PowerShell command in {session_id}: {command[:100]}...")
            ...
            self.log_info(f"PowerShell command completed in {execution_time:.2f}s with exit code {result.returncode}")
    ...
    async def open_program(self, session_id: str, program_path: str, arguments: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_info(f"Opening program in {session_id}: {program_path} {arguments}")
            
            # Execute command
            result = await self.execute_powershell_command(session_id, powershell_command)
    ...
    async def track_mouse_click(self, session_id: str, click_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log_info(f"Tracking mouse click in {session_id}: ({x}, {y}) {button} x{clicks}")
            ...
            while self.ocr_enabled and not self.ocr_stop_event.is_set():
                try:
                    ...
                    if ocr_text.strip():
                        ...
                        self.log_debug(f"OCR detected {len(ocr_text.strip())} characters")
    ...
    async def handle_websocket(self, websocket: WebSocket):
        ...
        self.log_info(f"🤖 [DesktopAutomation] Handling WebSocket connection from {client_host}")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    await self._handle_websocket_message(websocket, message, client_host)
                except WebSocketDisconnect:
                    self.log_info(f"🔌 [DesktopAutomation] Client {client_host} disconnected")
                    break
                except Exception as e:
                    self.log_error(f"💥 [DesktopAutomation] Message handling error for {client_host}: {e}")
                    break
                    
        except Exception as e:
            self.log_error(f"💥 [DesktopAutomation] WebSocket error for {client_host}: {e}")
        finally:
            # Remove from clients
            self.websocket_clients.discard(websocket)
            self.log_info(f"🔌 [DesktopAutomation] WebSocket connection closed for {client_host}")
    
    async def _handle_websocket_message(self, websocket: WebSocket, message: str, client_host: str):
        """Handle incoming WebSocket messages"""
        try:
            data = json.loads(message)
            message_type = data.get("type", "unknown")
            
            self.log_debug(f"📨 [DesktopAutomation] Received {message_type} from {client_host}")
            ...
            self.log_warning(f"⚠️  [DesktopAutomation] Invalid JSON from {client_host}: {message}")
        except Exception as e:
            self.log_error(f"💥 [DesktopAutomation] Error handling message from {client_host}: {e}")
            error_response = {
                "type": "error",
                "message": str(e),
                "timestamp": datetime.now().isoformat()
