...
async def test():
    ...
    # Test 1: Skill Models
    from app.models.skill import SkillCategory, SkillSummary
    ...
    print(
        f'[OK] 2. Search "browser": {result.total} results -> {[s.name for s in result.skills]}'
    )
    passed += 1
    ...
    print(
        f"[OK] 4. Detail: {detail.name} v{detail.version}, permissions={[p.value for p in detail.permissions]}"
    )
    passed += 1
    ...
    exec_result = await manager.execute_skill(
        "github-manager", {"command": "list repos"}
    )
    print(
        f"[OK] 9. Execute: success={exec_result.success}, time={exec_result.execution_time_ms:.1f}ms"
    )
    ...
    print(
        f"[OK] 11. Execute disabled: success={exec_disabled.success}, error={exec_disabled.error}"
    )
    ...
    print(
        f'[OK] 12. Stats: installed={stats["total_installed"]}, executions={stats["total_executions"]}'
    )
    passed += 1
    ...
    print(
        f"[OK] 14. Category filter (development): {[s.name for s in result_dev.skills]}"
    )
    passed += 1
