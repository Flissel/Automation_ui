...
class MoireWebSocketClient:
    ...
    async def _handle_message(self, msg: Dict[str, Any]):
        ...
        # Handle moire_detection_result - contains all boxes with OCR texts
            ...
            logger.info(
                f"moire_detection_result: {len(elements)} boxes, {self._last_ocr_text_count} with text"
            )
            ...
            if self._capture_state in [
                ...
                self._capture_event.set()
                logger.info(
                    f"Capture complete! {self._last_ocr_text_count} texts found"
                )
    ...
    def find_elements_in_region(
        ...
        for element in self.current_context.elements:
            ...
            # Überlappung prüfen
            if ex < x + width and ex + ew > x and ey < y + height and ey + eh > y:
                results.append(element)
...
"""
    ...
    async def capture_and_wait_for_complete(
        self, timeout: float = 120.0, min_ocr_confidence: float = 0.3
    ) -> CaptureResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Capture complete in {processing_time:.0f}ms: {self._last_boxes_count} boxes, {self._last_ocr_text_count} texts"
            )
    ...
    async def capture_with_retry(
        self, max_retries: int = 3, timeout_per_try: float = 60.0, min_texts: int = 1
    ) -> CaptureResult:
        """
        ...
        logger.warning(
            f"All {max_retries} capture attempts completed. Best result: {last_result.texts_count if last_result else 0} texts"
        )
    ...
    def is_capture_in_progress(self) -> bool:
        ...
        return self._capture_state not in [
            CaptureState.IDLE,
            CaptureState.COMPLETE,
            CaptureState.ERROR,
        ]
...
def get_moire_client(host: str = "localhost", port: int = 8766) -> MoireWebSocketClient:
...
async def main():
    ...
    client.on_state_changed(
        lambda e: print(
            f"State changed: v{e.version}, {len(e.ui_context.elements)} elements"
        )
    )
