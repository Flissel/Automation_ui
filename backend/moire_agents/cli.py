...
class InteractiveCLI:
    ...
    async def initialize(self):
        ...
        if self.real_execution:
            try:
                ...
                self._print_warning(
                    "PyAutoGUI not available - running in simulation mode"
                )
    ...
    def _print_banner(self):
        ...
        print(
            f"{Fore.CYAN}{Style.BRIGHT}   MoireTracker - Interactive Automation CLI{Style.RESET_ALL}"
        )
        print(f"{Fore.CYAN}{'=' * 60}{Style.RESET_ALL}")
        print()
        print(
            f"  {Fore.WHITE}Type natural language commands to automate your desktop.{Style.RESET_ALL}"
        )
        print(
            f"  {Fore.WHITE}Type 'help' for available commands, 'exit' to quit.{Style.RESET_ALL}"
        )
        print()
        if self.real_execution:
            print(
                f"  {Fore.GREEN}[REAL MODE]{Style.RESET_ALL} Actions will be executed on your desktop"
            )
        else:
            print(
                f"  {Fore.YELLOW}[SIMULATION]{Style.RESET_ALL} Actions are simulated (use --real for actual execution)"
            )
        print()
    ...
    def _print_progress(self, status: dict):
        ...
        if state == "decomposing":
            print(
                f"  {Fore.BLUE}[Analyzing]{Style.RESET_ALL} Breaking down your task..."
            )
            ...
            print(
                f"  {Fore.YELLOW}[Phase {phase}/{total_phases}]{Style.RESET_ALL} {bar} {progress:.0%}"
            )
        elif state == "executing_subtask":
            subtask = status.get("subtask", "unknown")
            approach = status.get("approach", "")
            print(
                f"    {Fore.MAGENTA}>{Style.RESET_ALL} {subtask} {Fore.DIM}[{approach}]{Style.RESET_ALL}"
            )
            ...
            if success:
                print(
                    f"  {Fore.GREEN}[Complete]{Style.RESET_ALL} Task finished in {duration:.1f}s"
                )
            else:
                print(
                    f"  {Fore.RED}[Failed]{Style.RESET_ALL} Task failed after {duration:.1f}s"
                )
        elif state == "failed":
    ...
    async def _execute_task(self, goal: str):
        ...
        try:
            ...
            if result.success:
                self._print_success(
                    f"Completed {result.subtasks_completed}/{result.subtasks_total} subtasks"
                )
                if result.summary:
    ...
    def _show_help(self):
        ...
        print(
            f"    {Fore.GREEN}> Search for Python tutorials on Google{Style.RESET_ALL}"
        )
        ...
        print(
            f"    {Fore.YELLOW}status{Style.RESET_ALL}   - Show status of current tasks"
        )
        ...
        print(
            f"    {Fore.YELLOW}mode{Style.RESET_ALL}     - Toggle real/simulation mode"
        )
    ...
    def _show_status(self):
        ...
        tasks = (
            self.interface.get_running_tasks()
            if hasattr(self.interface, "get_running_tasks")
            else []
        )
...
async def main():
    ...
    parser.add_argument(
        "--real", "-r", action="store_true", help="Enable real PyAutoGUI execution"
    )
