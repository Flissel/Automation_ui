...
from .selection_manager import (SelectionManager, SelectionSnapshot,
        ...
        return {
            "cursor": {
                ...
                "text_pos": (
                    ...
                    if self.cursor.has_text_position
                    else None
                ),
            },
            "selection": {
                ...
                "preview": (
                    self.selection.text[:50] + "..."
                    if self.selection.text and len(self.selection.text) > 50
                    else self.selection.text
                ),
            },
            "app": {
...
class ContextTracker:
    ...
    async def update_after_action(
        self, action_type: str, action_params: Optional[Dict[str, Any]] = None
    ) -> ContextState:
        """
        ...
        if action_type == "click":
            ...
            self._selection.start_x = (
                start[0] if isinstance(start, tuple) else start.get("x", 0)
            )
            self._selection.start_y = (
                start[1] if isinstance(start, tuple) else start.get("y", 0)
            )
            self._selection.end_x = (
                end[0] if isinstance(end, tuple) else end.get("x", 0)
            )
            self._selection.end_y = (
                end[1] if isinstance(end, tuple) else end.get("y", 0)
            )
            await self._capture_selection()
    ...
    async def _capture_selection(self) -> Optional[SelectionSnapshot]:
        ...
        try:
            snapshot = await self.selection_manager.capture_selection(
                interaction_agent=self.interaction_agent, source="context_tracker"
            )
    ...
    async def update_app_context_from_vision(
        ...
        try:
            ...
            analysis = await self.vision_agent.analyze_screenshot(
                image, context="Identifiziere die aktive Anwendung und UI-Zustand"
            )
            ...
            if analysis.success:
                ...
                # Prüfe auf Dialog
                if (
                    "dialog" in desc_lower
                    or "popup" in desc_lower
                    or "fenster" in desc_lower
                ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "app": {"type": self._app.app_type.value, "name": self._app.app_name},
