...
class WordHelper:
    ...
    async def select_paragraph(
        ...
        # Triple-Click
        if click_position:
            result = await self.interaction.click(target=click_position, clicks=3)
        else:
            result = await self.interaction.click(clicks=3)

        if result.get("success") and self.context:
            await self.context.update_after_action(
                "triple_click", {"position": click_position}
            )
    ...
    async def select_word(
        ...
        if result.get("success") and self.context:
            await self.context.update_after_action(
                "double_click", {"position": click_position}
            )
            selected = await self.context.get_selected_text()
            result["selected_text"] = selected
    ...
    async def extend_selection(
        self, direction: str, unit: str = "word"
    ) -> Dict[str, Any]:
        """
        ...
        if result.get("success") and self.context:
            await self.context.update_after_action(
                "extend_selection", {"direction": direction, "unit": unit}
            )
    ...
    async def apply_format(self, format_type: FormatType) -> Dict[str, Any]:
        ...
        if result.get("success"):
            ...
            if format_type == FormatType.BOLD:
                ...
                self._formatting_state.is_underline = (
                    not self._formatting_state.is_underline
                )
    ...
    async def format_paragraph_bold(
        self, paragraph_number: int = 1, via_vision: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if via_vision and self.vision and self._last_screenshot:
                ...
                location = await self.vision.find_element_from_screenshot(
                    self._last_screenshot, f"Der {paragraph_number}. Absatz im Dokument"
                )
            ...
            result["steps"].append(
                f"Selected paragraph: {select_result.get('char_count', 0)} chars"
            )
    ...
    async def format_selection(
        self, formats: List[FormatType], verify: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    async def verify_format_applied(
        self, format_type: FormatType, screenshot_bytes: Optional[bytes] = None
    ) -> bool:
        """
    ...
    async def get_ribbon_state(self, screenshot_bytes: bytes) -> FormattingState:
        ...
        try:
            ...
            ribbon_height = 150
            ribbon_image = image.crop(
                (0, 0, image.width, min(ribbon_height, image.height))
            )
            ...
            if analysis.success:
                ...
                # Parse state
                state.is_bold = "bold" in desc_lower and (
                    "active" in desc_lower or "pressed" in desc_lower
                )
                state.is_italic = "italic" in desc_lower and (
                    "active" in desc_lower or "pressed" in desc_lower
                )
                state.is_underline = "underline" in desc_lower and (
                    "active" in desc_lower or "pressed" in desc_lower
                )
