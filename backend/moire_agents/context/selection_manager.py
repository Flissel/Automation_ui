...
class SelectionManager:
    ...
    async def read_clipboard(self) -> ClipboardInfo:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.warning(
                    f"Clipboard read failed: {stderr.decode('utf-8', errors='ignore')}"
                )
                return ClipboardInfo(
                    state=ClipboardState.UNKNOWN, timestamp=time.time()
                )

            text = stdout.decode("utf-8", errors="ignore").strip()

            if not text:
                return ClipboardInfo(state=ClipboardState.EMPTY, timestamp=time.time())
    ...
    async def write_clipboard(self, text: str) -> bool:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(
                    f"Clipboard write failed: {stderr.decode('utf-8', errors='ignore')}"
                )
                return False
    ...
    async def capture_selection(
        self, interaction_agent: Any = None, source: str = "auto"
        ...
        try:
            ...
            if current.has_text and current.text:
                snapshot = SelectionSnapshot(
                    text=current.text, timestamp=time.time(), source=source
                )
                ...
                logger.info(
                    f"Selection captured: {snapshot.char_count} chars, {snapshot.word_count} words"
                )
                return snapshot
