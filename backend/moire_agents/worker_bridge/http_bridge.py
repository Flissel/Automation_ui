...
from .messages import ActionStep  # NEW: Tool-Using Agent Messages
from .messages import (BatchClassifyResult, ClassifyIconMessage,
                       ExecutionStatus, ReplanRequest, SizeValidationReport,
                       TaskContext, TaskExecutionRequest, TaskExecutionResult,
                       ToolExecutionResult, ToolName, ValidationResult)
...
try:
    from .workers.desktop_tools import (DESKTOP_TOOLS, get_tool_executor,
...
class HttpBridgeServer:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✓ HTTP Bridge Server gestartet auf http://{self.host}:{self.port}"
            )
            return True
    ...
    async def _handle_status(self, request: web.Request) -> web.Response:
        ...
        status = {
            ...
            "workers": {
                "totalWorkers": (
                    3 if HAS_EXECUTION_WORKER else 2
    ...
    async def _handle_classify_batch(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            logger.info(
                f"[{batch_id}] Batch Classification Request: {len(icons_data)} icons"
            )
            ...
            logger.info(
                f"[{batch_id}] Batch complete: {response['successful']}/{len(icons)} in {processing_time:.0f}ms"
            )
    ...
    async def _handle_al_confirm(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            logger.info(
                f"[ActiveLearning] Label confirmed: {box_id} → {confirmed_label}"
            )
    ...
    async def _handle_stats(self, request: web.Request) -> web.Response:
        ...
        clf_stats = (
            self._classification_worker.get_stats()
            if self._classification_worker
            else {}
        )
        val_stats = (
            self._validation_worker.get_stats() if self._validation_worker else {}
        )
        exec_stats = (
            self._execution_worker.get_stats() if self._execution_worker else {}
        )
    ...
    async def _handle_execute_action(self, request: web.Request) -> web.Response:
        ...
        if not HAS_EXECUTION_WORKER or not self._execution_worker:
            return self._cors_response({"error": "Execution Worker not available"}, 503)
        ...
        try:
            ...
            result = await self._execution_worker.execute_single_action(
                tool_name=tool_name, params=tool_params, context=context
            )
                ...
                tools_list.append(
                    {
                        ...
                        "requiresValidation": definition.get(
    ...
    async def _handle_task_status(self, request: web.Request) -> web.Response:
        ...
        if task_id in self._active_tasks:
            return self._cors_response(
                {"taskId": task_id, "status": "running", "inProgress": True}
            )
        ...
        return self._cors_response({"error": f"Task {task_id} not found"}, 404)

    # ==================== Internal Methods ====================

    async def _classify_icons(
        self, icons: List[ClassifyIconMessage]
        ...
        async def process_single(icon: ClassifyIconMessage) -> ValidationResult:
            async with semaphore:
                return await classify_and_validate(
                    icon, self._classification_worker, self._validation_worker
                )

        # Parallel processing
        results = await asyncio.gather(
            *[process_single(icon) for icon in icons], return_exceptions=True
        )
    ...
    def _add_to_al_queue(
        self, icon: ClassifyIconMessage, result: ValidationResult
    ) -> None:
