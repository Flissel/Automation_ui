...
try:
    ...
    HAS_GRPC = False
    logging.warning(
        "autogen-ext[grpc] not installed. Run: pip install autogen-ext[grpc]"
    )

from .messages import (BatchClassifyRequest, BatchClassifyResult,
                       ClassificationResult, ClassifyIconMessage,
                       ValidationRequest, ValidationResult, WorkerStatus,
                       WorkerType, calculate_combined_confidence,
...
class GrpcWorkerHost:
    ...
    def __init__(self, address: str = "localhost:50051", max_workers: int = 10):
        ...
        self._on_batch_complete_callbacks: List[
            Callable[[BatchClassifyResult], None]
        ] = []
    ...
    async def classify_batch(
        self, icons: List[ClassifyIconMessage], timeout: int = 30
    ) -> BatchClassifyResult:
        """
        ...
        logger.info(
            f"[{batch_id}] Starte Batch-Klassifizierung für {len(icons)} Icons..."
        )

        # Erstelle Batch Request
        batch_request = BatchClassifyRequest(
            batch_id=batch_id, icons=icons, timeout_seconds=timeout
        )
        ...
        logger.info(
            f"[{batch_id}] Batch fertig: {successful}/{len(icons)} erfolgreich in {processing_time:.0f}ms"
        )
...
Respond with ONLY the category name.""",
                ...
                categories_match = (
                    icon.cnn_category == llm_category if icon.cnn_category else False
                )
                ...
                needs_review = should_trigger_active_learning(
                    icon.cnn_category, llm_category, icon.cnn_confidence, 0.85
                )
                ...
                final_category = (
                    ...
                    else (icon.cnn_category or "unknown")
                )
    ...
    def on_batch_complete(
        self, callback: Callable[[BatchClassifyResult], None]
    ) -> None:
