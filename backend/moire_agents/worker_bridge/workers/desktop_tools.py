...
# Import Messages
from worker_bridge.messages import (ExecutionStatus, SizeValidationReport,
                                    SizeValidationResult, ToolExecutionResult,
                                    ToolName)
    ...
    ToolName.CAPTURE_SCREENSHOT_REGION: {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "width": {
                    "type": "integer",
                    "description": "Width of region (minimum 50px)",
                    "minimum": 50,
                },
                "height": {
                    "type": "integer",
                    "description": "Height of region (minimum 50px)",
                    "minimum": 50,
                },
                "target_element": {
                    "type": "string",
                    "description": "Optional: Element ID for size validation",
                },
            },
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "use_clipboard": {
                    "type": "boolean",
                    "default": True,
                    "description": "Use clipboard for Unicode support",
                },
            },
            "required": ["text"],
        },
    ...
    ToolName.PRESS_KEY: {
        ...
        "parameters": {
            ...
            "properties": {
                "key": {
                    "type": "string",
                    "description": "Key name (enter, tab, escape, up, down, left, right, space, backspace, delete, home, end, pageup, pagedown, f1-f12, win)",
                },
                "presses": {
                    "type": "integer",
                    "default": 1,
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "amount": {
                    "type": "integer",
                    "default": 3,
                    "description": "Scroll amount in lines",
                },
                "x": {
                    "type": "integer",
                    "description": "Optional: X position for scroll",
                },
                "y": {
                    "type": "integer",
                    "description": "Optional: Y position for scroll",
                },
            },
            "required": ["direction"],
        },
...
class SizeValidator:
    ...
    def validate_size_request(
        ...
        report = SizeValidationReport(
            result=result,
            original_request={"x": req_x, "y": req_y, "width": req_w, "height": req_h},
        ...
        logger.info(
            f"  Applied:  {applied_w}x{applied_h} at ({applied_x}, {applied_y})"
        )
        if reasons:
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        approved = sum(
            ...
            if r.result == SizeValidationResult.APPROVED
        )
        adjusted = sum(
            1
            for r in self.validation_history
            if r.result == SizeValidationResult.ADJUSTED
        )
        ...
        return {
            ...
            "adjusted": adjusted,
            "adjustment_rate": (
                adjusted / len(self.validation_history)
...
class DesktopToolExecutor:
    ...
    async def _capture_region_with_validation(
        self, params: Dict[str, Any], ui_state: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        if params.get("target_element") and ui_state:
            element = self._find_element(
                params["target_element"], ui_state.get("elements", [])
            )
            if element:
    ...
    async def _execute_click(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await self.interaction_agent.click(
            (x, y), button=button_enum, clicks=clicks
        )
    ...
    async def _execute_scroll(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await self.interaction_agent.scroll(
            direction, amount=amount, target=target
        )
    ...
    def _find_element(
        self, element_id: str, elements: List[Dict[str, Any]]
    ...
    def _log_execution(
        self, tool_name: ToolName, params: Dict[str, Any], result: Dict[str, Any]
    ):
...
async def main():
    ...
    # Test 3: Wait
    print("\n3. Wait Test")
    result = await executor.execute_tool(ToolName.WAIT, {"seconds": 0.5})
    print(f"   Success: {result.get('success')}")
