...
# Internal Imports
from worker_bridge.messages import (ActionStep, ExecutionStatus, ReplanRequest,
                                    SizeValidationReport, TaskContext,
                                    TaskExecutionRequest, TaskExecutionResult,
...
class ExecutionWorker:
    ...
    def __init__(
        ...
        self.client = openrouter_client or (
            get_openrouter_client() if HAS_OPENROUTER else None
        )
        ...
        self._on_step_complete: List[
            Callable[[ToolExecutionResult], Awaitable[None]]
        ] = []
        self._on_validation_failed: List[
            Callable[[ToolExecutionResult], Awaitable[None]]
        ] = []
        self._on_replan_requested: List[
            Callable[[ReplanRequest], Awaitable[List[ActionStep]]]
        ] = []
        ...
        logger.info(
            f"  Validation Threshold: {self.config.validation_threshold * 100}%"
        )

    def set_replan_callback(
        self, callback: Callable[[ReplanRequest], Awaitable[List[ActionStep]]]
    ):
        ...
        try:
            while current_plan and validation_rounds < request.max_validation_rounds:
                ...
                tool_result = await self.tool_executor.execute_tool(
                    step.tool_name, step.tool_params, ui_state=request.context.ui_state
                )
                ...
                if step_result.status == ExecutionStatus.SUCCESS:
                    logger.info(
                        f"  ✓ Step erfolgreich ({step_result.change_percentage:.1%} Änderung)"
                    )
                    ...
                    if validation_rounds < request.max_validation_rounds:
                        new_steps = await self._request_replan(
                            ...
                            remaining_rounds=request.max_validation_rounds
                            - validation_rounds,
                        )
                        ...
                        if new_steps:
                            ...
                            logger.info(
                                f"  Re-Planning erfolgreich: {len(new_steps)} neue Steps"
                            )
                        else:
                            logger.warning(f"  Re-Planning ergab keine neuen Steps")
                    else:
                        logger.error(
                            f"  Max Validation-Rounds erreicht ({request.max_validation_rounds})"
                        )

                elif step_result.status == ExecutionStatus.FAILED:
                    logger.error(
                        f"  ✗ Step fehlgeschlagen: {step_result.error_context}"
                    )
            ...
            return TaskExecutionResult(
                ...
                error_summary=(
                    None
                    if all_success
                    else f"{executed_steps - successful_steps} steps failed"
                ),
        ...
        if HAS_PIL:
            try:
                ...
                if change >= threshold:
                    ...
                    screenshot_after_retry = (
                        await self.tool_executor.capture_full_screen()
                    )

                    if screenshot_after_retry:
                        change_retry = self._compare_screenshots(
                            screenshot_before, screenshot_after_retry
                        )
                        ...
                        if change_retry >= threshold:
                            ...
                            result["error"] = (
                                f"Change {change_retry:.1%} below threshold {threshold:.1%}"
                            )
                    else:
                        result["status"] = ExecutionStatus.NEEDS_REPLAN
                        result["error"] = (
                            f"Change {change:.1%} below threshold {threshold:.1%}"
                        )
    ...
    def _compare_screenshots(
        self, screenshot_before: str, screenshot_after: str
    ) -> float:
        """
...
Bitte plane eine korrigierte Aktion um das Ziel zu erreichen."""
    ...
    async def execute_single_action(
        ...
        start_time = time.time()
        tool_result = await self.tool_executor.execute_tool(
            tool_name, params, ui_state=context.ui_state if context else None
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self._tasks_successful / self._tasks_processed
            ...
            "tool_executor_stats": (
                self.tool_executor.get_execution_stats() if self.tool_executor else {}
            ),
        }
...
if HAS_AUTOGEN:
    ...
    class ExecutionWorkerAgent(RoutedAgent):
        ...
        async def handle_execute_task(
            self, message: TaskExecutionRequest, ctx: MessageContext
        ) -> TaskExecutionResult:
...
async def main():
    ...
    print(
        f"Tasks: {stats['tasks_processed']} total, {stats['tasks_successful']} successful"
    )
