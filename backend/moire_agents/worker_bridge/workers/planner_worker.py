...
from worker_bridge.messages import (ActionStep, ExecutionStatus, ReplanRequest,
                                    TaskContext, TaskExecutionRequest,
...
class PlannerWorker:
    ...
    def __init__(
        ...
        logger.info(
            f"  OpenRouter: {'verfügbar' if self.client else 'nicht verfügbar'}"
        )
    ...
    async def create_plan(
        ...
        try:
            ...
            if len(action_steps) > self.config.max_planning_steps:
                logger.warning(
                    f"Plan hat {len(action_steps)} Steps, limitiere auf {self.config.max_planning_steps}"
                )
    ...
    async def create_replan(self, replan_request: ReplanRequest) -> List[ActionStep]:
        """
...
"""
    ...
    def _parse_tool_calls_to_steps(
        self, response: Any, prefix: str = "step"
    ) -> List[ActionStep]:
        ...
        for i, tool_call in enumerate(tool_calls):
            try:
                ...
                step = ActionStep(
                    ...
                    requires_validation=tool_name
                    not in [ToolName.WAIT, ToolName.WAIT_FOR_ELEMENT],
                )
    ...
    def _generate_expected_outcome(
        self, tool_name: ToolName, params: Dict[str, Any]
    ) -> str:
    ...
    def _create_fallback_plan(
        self, user_request: str, ui_state: Dict[str, Any]
    ) -> List[ActionStep]:
        ...
        # Type request
            ...
            "type" in request_lower
            or "text" in request_lower
            or "eingeben" in request_lower
        ):
...
async def main():
    ...
    steps = await planner.create_plan(
        "Klicke auf den Submit Button", ui_state, screen_bounds
    )
    ...
    print("\nTest 2: Type Request")
    steps = await planner.create_plan(
        'Gib "Hello World" in das Eingabefeld ein', ui_state, screen_bounds
    )
    print(f"  Generated {len(steps)} steps")
    for step in steps:
        print(f"    - {step.tool_name.value}: {step.expected_outcome}")
