...
from ..messages import (ClassificationResult, ClassifyIconMessage, UICategory,
                        WorkerType)

# NEU: CategoryRegistry Import
try:
    from services.category_registry import (CategoryRegistry,
...
class ClassificationWorker:
    ...
    def _get_category_registry(self) -> Optional[CategoryRegistry]:
        ...
        if self._category_registry is None and get_category_registry:
            ...
            logger.info(
                f"CategoryRegistry geladen: {len(self._category_registry.get_all_categories())} Kategorien"
            )
    ...
    def _get_system_prompt(self) -> str:
        ...
        if (
            ...
            and (now - self._prompt_cache_time).total_seconds()
            < self._prompt_cache_duration_seconds
        ):
    ...
    def _get_valid_categories(self) -> list:
        ...
        # Fallback
        return [cat.value for cat in UICategory] + [
            "browser",
            "editor",
            "system",
            "NEW_CATEGORY",
    ...
    async def _classify_internal(
        self, message: ClassifyIconMessage
        ...
        try:
                        ...
                        if new_cat_name and new_cat_desc:
                            ...
                            if registry:
                                ...
                                if result.get("action") == "approved":
                                    ...
                                    logger.info(
                                        f"✓ Neue Kategorie auto-approved: {new_cat_name}"
                                    )
                                    ...
                                    if new_cat_parent and registry.category_exists(
                                        new_cat_parent
                                    ):
                            ...
                            llm_category = "unknown"
                            reasoning = (
                                "NEW_CATEGORY suggested but missing name or description"
                            )
                    else:
                        # Validate category
            ...
            llm_confidence = (
                0.90 if semantic_name else (0.70 if llm_category != "unknown" else 0.30)
            )
    ...
    async def classify_batch(
        self, messages: list[ClassifyIconMessage]
        ...
        """
        logger.info(
            f"[{self.config.worker_id}] Batch-Klassifizierung: {len(messages)} Icons"
        )

        results = await asyncio.gather(
            *[self.classify(msg) for msg in messages], return_exceptions=True
        )
        ...
        logger.info(
            f"[{self.config.worker_id}] Batch fertig: {successful}/{len(messages)} erfolgreich"
        )
    ...
    def get_stats(self) -> dict:
        ...
        avg_time = (
            self._total_time_ms / self._tasks_processed
...
if HAS_AUTOGEN:
    ...
    class ClassificationWorkerAgent(RoutedAgent):
        ...
        async def handle_classify(
            self, message: ClassifyIconMessage, ctx: MessageContext
        ) -> ClassificationResult:
...
async def main():
    ...
    worker = ClassificationWorker(
        WorkerConfig(worker_id="test_worker", use_dynamic_categories=True)
    )
