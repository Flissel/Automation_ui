...
from ..messages import (ClassificationResult, ClassifyIconMessage, UICategory,
                        ValidationRequest, ValidationResult, WorkerType,
...
class VisionValidationWorker:
    ...
    async def validate(self, request: ValidationRequest) -> ValidationResult:
        ...
        final_confidence = calculate_combined_confidence(
            request.cnn_confidence, request.llm_confidence, categories_match
        )
        ...
        needs_review, add_to_training = self._check_active_learning(
            cnn_cat, llm_cat, request.cnn_confidence, request.llm_confidence
        )
    ...
    def _check_active_learning(
        self, cnn_cat: Optional[str], llm_cat: str, cnn_conf: float, llm_conf: float
    ) -> Tuple[bool, bool]:
        """
        ...
        if cnn_cat and cnn_cat != llm_cat:
            ...
            if (
                cnn_conf >= self.config.low_confidence_threshold
                and llm_conf >= self.config.low_confidence_threshold
            ):
                needs_review = True
    ...
    async def _trigger_active_learning(
        self, request: ValidationRequest, final_category: str
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            label = (
                final_category if final_category != "unknown" else request.llm_category
            )
            ...
            logger.info(
                f"[ActiveLearning] Sample hinzugefügt: {request.box_id} → {label}"
            )
            return label
    ...
    async def validate_batch(
        ...
        results = await asyncio.gather(*[self.validate(req) for req in requests])
        return list(results)
...
if HAS_AUTOGEN:
    ...
    class VisionValidationWorkerAgent(RoutedAgent):
        ...
        async def handle_validate(
            self, message: ValidationRequest, ctx: MessageContext
        ) -> ValidationResult:
...
async def main():
    ...
    for i, (cnn_cat, cnn_conf, llm_cat, llm_conf) in enumerate(test_cases):
        ...
        print(f"\nTest Case {i + 1}:")
        print(
            f"  Input:  CNN={cnn_cat} ({cnn_conf:.0%}), LLM={llm_cat} ({llm_conf:.0%})"
        )
