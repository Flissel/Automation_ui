...
class RLMemory:
    ...
    def hash_state(state_info: Dict[str, Any]) -> str:
        ...
        relevant = {
            "texts": sorted(
                [t.get("text", "")[:50] for t in state_info.get("texts", [])[:20]]
            ),
            "box_count": len(state_info.get("boxes", [])),
            "categories": sorted(
                list(set(b.get("category", "") for b in state_info.get("boxes", [])))
            ),
        }
    ...
    def end_episode(
        self, episode_id: int, success: bool, terminal_state: Optional[Dict] = None
    ) -> float:
        ...
        logger.info(
            f"Episode {episode_id} ended: success={success}, reward={total_reward:.2f}, steps={total_steps}"
        )
    ...
    def record_transition(
        ...
        self._update_q_value(
            state_hash, state_summary, action_key, action_text, reward, next_state_hash
        )
    ...
    def record_feedback(
        ...
        if transition_id and corrected_reward is not None:
            ...
            cursor.execute(
                "SELECT * FROM rl_transitions WHERE id = ?", (transition_id,)
            )
            row = cursor.fetchone()
            if row:
                action_key = self.action_key(
                    row["action_type"], json.loads(row["action_params"] or "{}")
                )
                self._update_q_value(
                    row["state_hash"],
                    row["state_summary"],
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        cursor.execute(
            "SELECT AVG(total_reward) as avg FROM rl_episodes WHERE end_time IS NOT NULL"
        )
    ...
    def get_learning_curve(self, window: int = 10) -> List[Dict[str, Any]]:
        ...
        for i, row in enumerate(episodes):
            ...
            avg_reward = sum(e["total_reward"] for e in window_episodes) / len(
                window_episodes
            )
            success_rate = sum(1 for e in window_episodes if e["success"]) / len(
                window_episodes
            )
...
def calculate_step_reward(
    action_result: str, verification: Dict[str, Any], step_text: str
...
if __name__ == "__main__":
    ...
    state2 = {
        "texts": [{"text": "Desktop"}, {"text": "This PC"}],
        "boxes": [{"category": "icon"}, {"category": "button"}],
    }
