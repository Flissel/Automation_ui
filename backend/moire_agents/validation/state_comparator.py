...
class StateComparator:
    ...
    def compare(self, state1: ScreenState, state2: ScreenState) -> ComparisonResult:
        """
        Vergleicht zwei Bildschirmzustände.
        ...
        if (
            ...
            and state2.screenshot_data
            and PIL_AVAILABLE
            and NUMPY_AVAILABLE
        ):
            change_percentage, changed_regions = self._compare_pixels(
                state1.screenshot_data, state2.screenshot_data
            )

        # Element-Vergleich
        new_elements, removed_elements = self._compare_elements(
            state1.elements, state2.elements
        )

        # Text-Vergleich
        text_changes = self._compare_text(state1.ocr_text, state2.ocr_text)
        ...
        description = self._create_description(
            change_type, change_percentage, new_elements, removed_elements, text_changes
        )
    ...
    def compare_with_roi(
        self, before_data: bytes, after_data: bytes, roi: Dict[str, Any]
    ) -> ComparisonResult:
        """
    ...
    def _compare_elements(
        self, elements1: List[Dict[str, Any]], elements2: List[Dict[str, Any]]
    ...
    def _compare_text(
        self, text1: List[str], text2: List[str]
    ) -> List[Tuple[str, str]]:
        """Vergleicht OCR-Text."""
        changes = []
    ...
    def has_state_changed_since(
        ...
        current_state = ScreenState.from_screenshot(
            current_data, elements=current_elements
        )
