...
from validation.state_comparator import (ChangeType, ComparisonResult,
                                         ScreenState, StateComparator,
...
class ActionValidator:
    ...
    async def validate_action(
        ...
        while time.time() - start_time < timeout:
            ...
            if roi and screenshot_before:
                ...
                last_comparison = self.comparator.compare_with_roi(
                    screenshot_before, screenshot_after, roi
                )
                logger.info(
                    f"ROI-Validierung: zoom={roi.get('zoom', 1.5)} um ({roi.get('origin_x', 0)}, {roi.get('origin_y', 0)})"
                )
            else:
                # Standard: Ganzer Screen-Vergleich
                last_comparison = self.comparator.has_state_changed_since(
                    reference_state, screenshot_after
                )
                ...
                if self._is_relevant_change(last_comparison, action, expected_change):
                    ...
                    if self.use_llm_validation and confidence < 0.9:
                        llm_result = await self._llm_validate(
                            action, expected_change, screenshot_before, screenshot_after
                        )
                        if llm_result:
                            confidence = max(
        ...
        return ValidationResult(
            ...
            change_type=(
                last_comparison.change_type if last_comparison else ChangeType.NO_CHANGE
            ),
            change_percentage=(
                last_comparison.change_percentage if last_comparison else 0.0
            ),
            duration_ms=duration_ms,
            attempts=attempts,
            screenshot_before=screenshot_before,
    ...
    def _is_relevant_change(
        self, comparison: ComparisonResult, action: ActionEvent, expected_change: str
    ) -> bool:
        ...
        if action.action_type == "type":
            return (
                comparison.change_type == ChangeType.TEXT_CHANGED
                or len(comparison.text_changes) > 0
                or comparison.change_percentage > 0.01
            )
    ...
    async def create_validation_event(
        self, action: ActionEvent, result: ValidationResult
    ) -> ValidationEvent:
