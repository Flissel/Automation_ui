...
async def main(task: Optional[str] = None, interactive: bool = True):
    ...
    try:
        if moire_client.is_connected:
            ...
            initial_result = await moire_client.capture_and_wait_for_complete(
                timeout=10.0
            )
            if initial_result.success:
                ...
                if initial_result.ocr_text:
                    logger.info(
                        f"  OCR: {len(initial_result.ocr_text)} Zeichen erkannt"
                    )
                if hasattr(initial_result, "boxes") and initial_result.boxes:
                    logger.info(
                        f"  Detection: {len(initial_result.boxes)} Elemente gefunden"
                    )
                ...
                if initial_result.screenshot_base64:
                    ...
                    orchestrator.set_initial_state(
                        ...
                        state={
                            ...
                            "boxes": (
                                initial_result.boxes
                                if hasattr(initial_result, "boxes")
                                else []
                            ),
                            "timestamp": time.time(),
                        },
    ...
    logger.info(
        f"MoireServer: {'✓ Connected' if status['has_moire_client'] else '✗ Disconnected'}"
    )
    logger.info(
        f"Interaction Agent: {'✓ Ready' if status['has_interaction_agent'] else '✗ Not available'}"
    )
    logger.info(f"========================\n")
...
def run():
    ...
    # Run
    try:
        asyncio.run(main(task=args.task, interactive=not args.no_interactive))
    except KeyboardInterrupt:
        logger.info("\nInterrupted by user")
