...
# WebSocket Client
from bridge.websocket_client import MoireWebSocketClient, UIContext, UIElement
...
class DesktopAnalyzer:
        ...
        try:
            ...
            async with self._session.get(f"{self.bridge_url}/status") as resp:
                if resp.status == 200:
                    ...
                    logger.warning(
                        "HTTP Bridge nicht erreichbar - nur OCR, keine LLM Names"
                    )
    ...
    async def scan_and_analyze(
        self, use_llm_names: bool = True, timeout: float = 120.0
    ) -> AnalysisResult:
        """
        ...
        logger.info(
            f"✓ {capture_result.boxes_count} Elemente erkannt, {capture_result.texts_count} mit OCR"
        )
        ...
        logger.info(
            f"✓ Analyse abgeschlossen: {len(elements)} Elemente, {named_count} benannt"
        )
    ...
    async def _classify_elements_with_llm(
        self, elements: List[AnalyzedElement], screenshot_base64: Optional[str]
        ...
        for elem in elements:
            ...
            if screenshot_base64 and HAS_PIL:
                crop_base64 = self._extract_crop(
                    screenshot_base64, elem.x, elem.y, elem.width, elem.height
                )
                    ...
                    if resp.status == 200:
                        ...
                        for result in data.get("results", []):
                            ...
                            if box_id in element_map:
                                ...
                                final_category = result.get(
                                    "finalCategory", elem.category
                                )
                        ...
                        logger.info(
                            f"  Batch {i//self.batch_size + 1}: {len(batch)} klassifiziert"
                        )
                    else:
                        logger.warning(
                            f"  Batch {i//self.batch_size + 1} fehlgeschlagen: {resp.status}"
                        )
    ...
    def _generate_semantic_name(
        self, llm_category: str, ocr_text: Optional[str], category: str
    ) -> str:
    ...
    def _extract_crop(
        self, screenshot_base64: str, x: int, y: int, width: int, height: int
    ) -> Optional[str]:
    ...
    try:
        async with DesktopAnalyzer() as analyzer:
            ...
            if result.success:
                ...
                print(f"\n{'='*60}")
                print(
                    f"ERGEBNIS: {result.total_elements} Elemente, {result.named_elements} benannt"
                )
