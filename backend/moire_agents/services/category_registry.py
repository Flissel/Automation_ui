...
class PendingCategory:
    ...
    )  # Element-Kontexte die zur Suggestion führten
...
class CategoryRegistry:
    ...
    def _load(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"CategoryRegistry geladen: {len(self._categories)} Kategorien, {len(self._pending)} pending"
            )
            return True
    ...
    def _save(self) -> bool:
        ...
        try:
            ...
            data = {
                ...
                "created": self._categories.get("button", Category("", "")).created_at
    ...
    def build_classification_prompt(
        ...
        sorted_cats = sorted(
            self._categories.values(), key=lambda c: (0 if c.parent else 1, c.name)
        )
        ...
        if "unknown" in self._categories:
            cat_lines.append(
                f"- **unknown**: {self._categories['unknown'].description}"
            )
