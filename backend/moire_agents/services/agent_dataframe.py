...
def _get_category_descendants(
    category: str, hierarchy: Dict[str, List[str]]
) -> Set[str]:
    """
...
class AgentDataFrame:
        ...
        for elem in self._elements:
            ...
            if elem.text:
                text_score = SequenceMatcher(
                    None, query_lower, elem.text.lower()
                ).ratio()
    ...
    def by_category_tree(self, parent_category: str) -> List[UIElement]:
        ...
        all_categories = _get_category_descendants(
            parent_category, self._category_hierarchy
        )
    ...
    def category_stats(self) -> Dict[str, Any]:
        ...
        """
        used_cats = {
            cat: len(elems) for cat, elems in self._by_category.items() if elems
        }
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        available = (
            ...
            if hasattr(self, "_registry_categories")
            else []
        )
    ...
    def at_point(self, x: int, y: int) -> List[UIElement]:
        ...
        for elem in self._grid[cell_key]:
            ...
            if (
                elem.x <= x <= elem.x + elem.width
                and elem.y <= y <= elem.y + elem.height
            ):
                result.append(elem)
    ...
    def in_region(self, x: int, y: int, width: int, height: int) -> List[UIElement]:
        ...
        for elem in candidates:
            ...
            if x <= elem.center_x <= x + width and y <= elem.center_y <= y + height:
                result.append(elem)

        return result

    def nearest(
        self, x: int, y: int, category: Optional[str] = None, max_distance: int = 500
    ) -> Optional[UIElement]:
        """
        ...
        elements = (
            self._by_category.get(category, self._elements)
            if category
            else self._elements
        )
    ...
    def summary(self) -> str:
        ...
        registry_info = (
            f", registry: {len(self._registry_categories)} categories"
            if self._registry_categories
            else ""
        )
        return (
            f"AgentDataFrame: {len(self)} elements, used: {cat_counts}{registry_info}"
        )
    ...
    def category_summary(self) -> str:
        ...
        used = sorted(
            stats["used_categories"].items(), key=lambda i: i[1], reverse=True
        )
        ...
        return (
            f"Categories: {len(self._registry_categories)} / {len(self._by_category)} (used), "
