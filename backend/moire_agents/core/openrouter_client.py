...
# Fallback-Modelle wenn vibemind_shared nicht verfuegbar
_MODEL_FALLBACKS = {
    "reasoning": "openrouter/free",
    "vision": "openrouter/free",
    "vision_fast": "openrouter/free",
    "quick": "openrouter/free",
}


class ModelType(Enum):
    """Verfügbare Modelle via OpenRouter — Werte aus llm_config.yml."""
    ...
    def model_id(self) -> str:
        """Modell-ID aus zentraler Config oder Fallback."""
        if _HAS_SHARED_CONFIG:
            try:
                return _shared_get_model(self.value)
            except Exception:
                pass
        return _MODEL_FALLBACKS.get(self.value, "anthropic/claude-sonnet-4")
...
class OpenRouterClient:
    ...
    async def chat(
        ...
        model_name = model.model_id if isinstance(model, ModelType) else model
        ...
        try:
            async with self.session.post(
                ...
                return LLMResponse(
                    content=content, model=model_name, usage=usage, raw_response=data
                )
    ...
    async def plan_actions(
        ...
        if HAS_LOCALIZATION and L:
            ...
            user_prompt = L.get(
                "plan_actions_user",
                goal=goal,
                screen_state=json.dumps(screen_state, indent=2, ensure_ascii=False)[
                    :3000
                ],
...
Create the action plan as JSON array:"""
        ...
        try:
            ...
            logger.error(
                f"Failed to parse action plan: {e}\nResponse: {response.content}"
            )
            return []
...
Respond as JSON:
{
  ...
  "suggestions": ["Possible next actions..."]
        ...
        try:
            ...
            return {"analysis": response.content, "elements": [], "suggestions": []}
...
{{"success": true/false, "confidence": 0.0-1.0, "description": "..."}}"""
        ...
        response = await self.chat_with_vision(
            prompt=prompt, image_data=after_screenshot, model=ModelType.VISION
        )
