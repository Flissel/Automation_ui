...
async def test():
    ...
    try:
        ...
        for name in test_names:
            ...
            if elem:
                print(
                    f"  df['{name}'] -> {elem.name} @({elem.center_x},{elem.center_y})"
                )
            else:
                ...
                if found:
                    print(
                        f"  df['{name}'] not found, but search found: {found[0].name}"
                    )
                else:
        ...
        if nearest:
            print(
                f"    nearest(960,540): {nearest.name} @({nearest.center_x},{nearest.center_y})"
            )
        ...
        print(
            f"  10000 x df[name] lookups: {elapsed:.2f}ms ({elapsed/10:.4f}ms per lookup)"
        )
