...
class InteractionAgent:
    ...
    def _get_coords(
        self, target: Union[Tuple[int, int], Dict[str, Any], str, None]
    ...
    async def click(
        ...
        try:
            ...
            self._record_action(
                "click", {"coords": coords, "button": button.value, "clicks": clicks}
            )
            ...
            if coords:
                action_type = (
                    "double_click"
                    if clicks == 2
                    else ("right_click" if button == MouseButton.RIGHT else "click")
                )
                await self._report_to_moire(
                    action_type, coords[0], coords[1], button=button.value
                )
            ...
            return {
                ...
                "data": {"coords": coords, "button": button.value, "clicks": clicks},
            }
    ...
    async def double_click(
        self, target: Union[Tuple[int, int], Dict[str, Any], None] = None
    ) -> Dict[str, Any]:
    ...
    async def move_to(
        self, target: Union[Tuple[int, int], Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    async def drag(
        ...
        try:
            ...
            self._record_action(
                "drag",
                {"start": start_coords, "end": end_coords, "button": button.value},
            )
        ...
        try:
            ...
            if use_clipboard or has_unicode:
                ...
                if result["success"]:
                    ...
                    self._record_action(
                        "type",
                        {"text": text[:50], "length": len(text), "method": "clipboard"},
                    )
            ...
            self._record_action(
                "type", {"text": text[:50], "length": len(text), "method": "direct"}
            )
    ...
    async def type_text_chunked(
        self, text: str, chunk_size: int = 500, chunk_delay: float = 0.5
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._record_action(
                "type_chunked", {"total_length": len(text), "chunks": len(chunks)}
            )
            ...
            return {
                ...
                "action": "type_chunked",
                "duration_ms": duration_ms,
                "data": {"text_length": total_chars, "chunks": len(chunks)},
            }
    ...
    async def press_key(
        self, key: str, presses: int = 1, interval: float = 0.1
    ) -> Dict[str, Any]:
        """
        Drückt eine Taste.
    ...
    async def scroll(
        ...
        try:
            ...
            self._record_action(
                "scroll",
                {"direction": direction.value, "amount": amount, "coords": coords},
            )
    ...
    async def select_text_by_coords(
        ...
        try:
            ...
            self._record_action("select_text", {"start": start, "end": end})
    ...
    async def replace_selected_text(self, new_text: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._record_action(
                "replace_text", {"new_text": new_text[:50], "length": len(new_text)}
            )
    ...
    async def select_and_replace(
        self, start: Tuple[int, int], end: Tuple[int, int], new_text: str
    ) -> Dict[str, Any]:
        """
    ...
    async def select_element_and_replace(
        self, element: Dict[str, Any], new_text: str
    ) -> Dict[str, Any]:
        """
