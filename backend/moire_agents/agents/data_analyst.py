...
class CompleteAnalysisResult:
    ...
    def to_agent_context(self) -> str:
        ...
        if len(self.ui_context.available_actions) > 10:
            lines.append(
                f"  ... und {len(self.ui_context.available_actions) - 10} weitere"
            )
...
class DataAnalystAgent:
    ...
    def merge_csv_results(self) -> MergedCSVData:
        ...
        for box in boxes:
            ...
            for region in regions:
                if (
                    region["min_x"] <= box_center_x <= region["max_x"]
                    and region["min_y"] <= box_center_y <= region["max_y"]
                ):
        ...
        for box in boxes:
            ...
            for line in lines:
                if line["region_id"] != box_region:
                    continue
                if (
                    line["min_x"] <= box_center_x <= line["max_x"]
                    and line["min_y"] <= box_center_y <= line["max_y"]
                ):
        ...
        avg_confidence = (
            sum(b["confidence"] for b in boxes) / total_boxes if total_boxes > 0 else 0
        )
    ...
    async def wait_and_analyze(
        ...
        if retry_on_low_quality:
            capture_result = await client.capture_with_retry(
                max_retries=3, timeout_per_try=timeout / 3, min_texts=1
            )
        else:
    ...
    def should_use_vision(
        ...
        if ocr_quality < self.min_ocr_quality:
            return (
                True,
                f"Niedrige OCR-Qualität ({ocr_quality:.0%} < {self.min_ocr_quality:.0%})",
            )
        ...
        if merged_data.avg_confidence < self.min_confidence:
            return (
                True,
                f"Niedrige Konfidenz ({merged_data.avg_confidence:.0%} < {self.min_confidence:.0%})",
            )
    ...
    def _create_readable_summary(
        ...
        lines.append(
            f"   • Durchschnittliche Konfidenz: {merged_data.avg_confidence:.0%}"
        )
        ...
        if ui_context.available_actions:
            lines.append(
                f"⚡ Verfügbare Aktionen ({len(ui_context.available_actions)}):"
            )
    ...
    def analyze_ui_context(self, raw_context: Dict[str, Any]) -> UIScreenContext:
        """
        ...
        summary = self._create_summary(
            toolbar_elems, menu_elems, content_elems, statusbar_elems, available_actions
        )
        ...
        logger.info(
            f"Analyzed UI: {context.element_count} elements, {len(available_actions)} actions"
        )

        return context

    def _analyze_element(
        self, elem: Dict[str, Any], regions: List[Dict], lines: List[Dict]
    ) -> AnalyzedElement:
        ...
        return AnalyzedElement(
            ...
            bounds=bounds,
            center=(
                elem.get("center", {}).get("x", 0),
                elem.get("center", {}).get("y", 0),
            ),
    ...
    def _determine_element_type(
        self, text: Optional[str], width: int, height: int, bounds: Dict[str, int]
    ) -> UIElementType:
    ...
    def _determine_semantic_type(
        self, text: Optional[str], elem_type: UIElementType
    ) -> Optional[str]:
        ...
        if any(
            w in text_lower
            for w in ["back", "forward", "next", "previous", "zurück", "weiter"]
        ):
            return "navigation"
        ...
        if any(
            w in text_lower
            for w in ["ok", "yes", "confirm", "apply", "ja", "bestätigen"]
        ):
            return "confirmation"
        ...
        if any(
            w in text_lower
            for w in ["cancel", "no", "close", "abbrechen", "nein", "schließen"]
        ):
            return "cancellation"
    ...
    def _determine_function(
        self, text: Optional[str], elem_type: UIElementType, bounds: Dict[str, int]
    ) -> Optional[str]:
    ...
    def _extract_actions(self, elements: List[AnalyzedElement]) -> List[Dict[str, Any]]:
        ...
        for elem in elements:
            ...
            if elem.text:
                ...
                action["description"] = (
                    f"{elem.action_type} at ({elem.center[0]}, {elem.center[1]})"
                )
    ...
    def _detect_application(self, elements: List[AnalyzedElement]) -> Optional[str]:
        ...
        toolbar_texts = [
            e.text.lower() for e in elements if e.text and e.bounds.get("y", 1000) < 100
        ]
    ...
    def _detect_window_title(self, elements: List[AnalyzedElement]) -> Optional[str]:
    ...
    def _detect_current_view(self, elements: List[AnalyzedElement]) -> Optional[str]:
    ...
    async def find_element(
        self, description: str, ui_context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if (
            not self.current_context
            or self.current_context.timestamp != ui_context.get("timestamp")
        ):
    ...
    async def verify_state(
        self, condition: Optional[str], ui_context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if not ui_context:
            return {
                "success": True,
                "data": {"verified": False, "reason": "No UI context"},
            }
        ...
        all_texts = []
        for elem in (
            context.toolbar_elements
            + context.menu_elements
            + context.content_elements
            + context.statusbar_elements
        ):
            if elem.text:
