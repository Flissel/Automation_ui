...
class MonitorAgent:
    ...
    def _compare_states(
        self, old_state: MonitorState, new_state: MonitorState
    ) -> ChangeEvent:
        """Vergleicht zwei States und ermittelt Änderungen."""
        affected_elements = []
        delta = {"added": [], "removed": [], "modified": [], "text_changed": False}
        ...
        return {
            **self.stats,
            ...
            "current_state_version": (
                self.current_state.version if self.current_state else None
            ),
        }
    ...
    def get_change_summary(self) -> str:
        ...
        lines.append(
            f"Checks: {self.stats['checks_performed']}, Änderungen: {self.stats['changes_detected']}"
        )
        lines.append("")
    ...
    async def wait_for_change(
        self, timeout: float = 30.0, min_significance: float = 0.0
    ...
    async def wait_for_stable_state(
        self, stability_duration: float = 2.0, timeout: float = 30.0
    ) -> bool:
        """
