...
class VisionAnalystAgent:
    ...
    def is_available(self) -> bool:
        ...
        return (
            self.openrouter_client is not None or self.client is not None
        ) and HAS_PIL

    async def find_element(
        self, image: "PILImage.Image", element_description: str, context: str = ""
    ) -> ElementLocation:
        """
...
{{
    ...
    "description": "Element not found: <reason>"
            ...
            if self.openrouter_client:
                response = await self.openrouter_client.chat_with_vision(
                    prompt=prompt, image_data=base64_image, json_mode=True
                )
                ...
                if response and response.content:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse vision response: {response.content[:200]}"
                        )

            # Fallback to Anthropic if available
            elif self.client:
                response = await asyncio.to_thread(
                    self._call_claude_vision, base64_image, prompt
                )
    ...
    async def find_element_from_screenshot(
        self, screenshot_bytes: bytes, element_description: str, context: str = ""
    ) -> ElementLocation:
        """
    ...
    async def analyze_screen_for_task(
        self, image: "PILImage.Image", task_description: str
    ) -> Dict[str, Any]:
        """
...
{{
                ...
                if response and response.content:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and "x" in result["suggested_action"]
                        ):
                            if max(original_size) > max_size:
                                scale = original_size[0] / image.size[0]
                                result["suggested_action"]["x"] = int(
                                    result["suggested_action"]["x"] * scale
                                )
                                result["suggested_action"]["y"] = int(
                                    result["suggested_action"]["y"] * scale
                                )
    ...
    async def analyze_screenshot(
        ...
        try:
            ...
            if self.openrouter_client:
                response = await self.openrouter_client.chat_with_vision(
                    prompt=prompt, image_data=base64_image
                )
                if response and response.content:
...
Formatiere die Antwort strukturiert mit klaren Abschnitten."""
    ...
    def _parse_vision_response(self, response: str) -> VisionAnalysisResult:
        ...
        for line in lines:
            ...
            if "button" in line_lower or "schaltfläche" in line_lower:
                ...
                "textfeld" in line_lower
                or "eingabe" in line_lower
            ...
            if line.strip().startswith("-") or line.strip().startswith("•"):
                ...
                if current_section == "action":
                    suggested_actions.append(
                        {"description": item_text, "confidence": 0.7}
                    )
    ...
    async def analyze_from_client(
        self, client: "MoireWebSocketClient", context: str = ""
    ) -> VisionAnalysisResult:
        """
    ...
    def create_multimodal_message(
        self, image: "PILImage.Image", text: str = "Analysiere diesen Screenshot"
        ...
        try:
            ...
            return MultiModalMessage(
                content=[text, autogen_image], source="vision_agent"
            )
        except Exception as e:
    ...
    async def describe_element(
        self, image: "PILImage.Image", element_bounds: Dict[str, int]
    ) -> str:
        """
    ...
    async def analyze_with_prompt(self, screenshot: bytes, prompt: str) -> str:
        """
            ...
            for line in lines:
                ...
                if line_lower.startswith("ziel_erreicht:"):
                    ...
                    if problems_str and problems_str.lower() != "keine":
                        issues = [
                            p.strip() for p in problems_str.split("|") if p.strip()
                        ]
                    ...
                    if corrections_str and corrections_str.lower() != "keine":
                        corrections = [
                            c.strip() for c in corrections_str.split("|") if c.strip()
                        ]
...
async def test_vision_agent():
    ...
    if agent.is_available() and HAS_PIL:
        ...
        result = await agent.analyze_screenshot(test_image, context="Test screenshot")

        print(f"Analysis result: {result.success}")
        print(
            f"Description: {result.description[:200] if result.description else 'None'}..."
        )
