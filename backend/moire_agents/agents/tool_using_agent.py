...
try:
    from worker_bridge.messages import (ActionStep, ExecutionStatus,
                                        ReplanRequest, TaskContext,
                                        TaskExecutionRequest,
                                        TaskExecutionResult,
                                        ToolExecutionResult, ToolName)
    from worker_bridge.workers.desktop_tools import DESKTOP_TOOLS
    from worker_bridge.workers.execution_worker import (ExecutionWorker,
                                                        get_execution_worker)
    from worker_bridge.workers.planner_worker import (PlannerWorker,
                                                      get_planner_worker)
...
class ToolUsingAgent:
    ...
    def __init__(
        ...
        if (
            ...
            and self.config.integration_mode == "orchestrator"
        ):
        ...
        self._on_task_complete: List[
            Callable[[TaskExecutionResult], Awaitable[None]]
        ] = []
        ...
        logger.info(
            f"  Worker Bridge: {'verfügbar' if HAS_WORKER_BRIDGE else 'nicht verfügbar'}"
        )
        logger.info(
            f"  Orchestrator: {'verbunden' if self._orchestrator else 'nicht verbunden'}"
        )
    ...
    async def execute_natural_language_task(
        ...
        """
        if (
            not HAS_WORKER_BRIDGE
            or not self._execution_worker
            or not self._planner_worker
        ):
            return self._create_error_result(
                "Worker Bridge nicht verfügbar", user_request
            )
        ...
        try:
            ...
            if not action_plan:
                return self._create_error_result(
                    "Planner konnte keinen Plan erstellen", user_request, task_id
                )
    ...
    async def execute_predefined_plan(
        self, action_plan: List[ActionStep], context: TaskContext
    ) -> TaskExecutionResult:
        """
        ...
        if not HAS_WORKER_BRIDGE or not self._execution_worker:
            return self._create_error_result(
                "Worker Bridge nicht verfügbar", context.user_request
            )
    ...
    async def execute_single_action(
        ...
        return await self._execution_worker.execute_single_action(
            tool_name=tool_name_enum, params=params, context=context
        )

    async def _handle_replan(self, replan_request: ReplanRequest) -> List[ActionStep]:
        """
    ...
    def _create_error_result(
        self, error: str, user_request: str, task_id: Optional[str] = None
    ) -> TaskExecutionResult:
    ...
    def on_task_complete(
        self, callback: Callable[[TaskExecutionResult], Awaitable[None]]
    ):
        """Registriert Callback für Task-Abschluss."""
        self._on_task_complete.append(callback)

    def on_step_complete(
        self, callback: Callable[[ToolExecutionResult], Awaitable[None]]
    ):
    ...
    async def execute_with_orchestrator_reflection(
        self, goal: str, context: Optional[Dict[str, Any]] = None, max_rounds: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        result = await self._orchestrator.execute_task_with_reflection(
            goal=goal, context=context, max_reflection_rounds=max_rounds
        )
