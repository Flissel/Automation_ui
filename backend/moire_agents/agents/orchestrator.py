...
class ClaudeCLIWrapper:
        ...
        for path in possible_paths:
            try:
                ...
                result = subprocess.run(
                    (
                        [path, "--version"]
                        if not path.startswith("npx")
                        else path.split() + ["--version"]
                    ),
                    capture_output=True,
                    text=True,
                    timeout=5,
    ...
    async def run_command(
        ...
        try:
            ...
            cmd = (
                self.claude_path.split()
                if "npx" in self.claude_path
                else [self.claude_path]
            )
    ...
    async def run_skill(
        ...
        context = {"skill_inputs": inputs, "ui_context": ui_context}
        ...
        # Baue Prompt
        prompt = (
            f"Execute skill '{skill_name}' with the provided inputs and UI context."
        )
...
"""
    ...
    def _plan_steps_fallback(self, goal: str) -> List[Dict[str, Any]]:
        ...
        steps.append(
            {"action": "capture", "description": "Aktuellen Bildschirm erfassen"}
        )
        ...
        if "öffne" in goal_lower or "open" in goal_lower:
            ...
            steps.append(
                {"action": "click", "description": "Auf gefundenes Element klicken"}
            )
            ...
            steps.append({"action": "click", "description": "Klick ausführen"})
            ...
            steps.append({"action": "verify", "description": "Zustand analysieren"})
    ...
    async def execute_task(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            while task.current_step < len(task.steps):
                ...
                logger.info(
                    f"Executing step {task.current_step + 1}/{len(task.steps)}: {step.get('description', step['action'])}"
                )
        ...
        if action == "capture":
            ...
            if "data_analyst" in self.agents:
                return await self.agents["data_analyst"].find_element(
                    step.get("target"), context.get("ui_context")
                )
            ...
            if "interaction" in self.agents:
                return await self.agents["interaction"].scroll(
                    step.get("direction", "down"), step.get("amount", 3)
                )
            return {"success": False, "error": "Interaction agent not available"}
            ...
            if "data_analyst" in self.agents:
                return await self.agents["data_analyst"].verify_state(
                    step.get("condition"), context.get("ui_context")
                )
            ...
            result = await self.claude_cli.run_skill(
                skill_name, inputs, context.get("ui_context")
            )
    ...
    async def handle_user_request(
        self, request: str, ui_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Erstelle Task
        task = await self.create_task(goal=request, context={"ui_context": ui_context})
