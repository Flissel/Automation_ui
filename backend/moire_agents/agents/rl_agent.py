...
from memory.rl_memory import (Episode, HumanFeedback, QTableEntry,
                              RewardSource, RLMemory, Transition,
                              calculate_step_reward, get_rl_memory)
...
class RLAgent:
    ...
    def start_task(self, task_description: str, task_id: str = "") -> Episode:
        ...
        logger.info(
            f"RL Episode {self._current_episode.id} gestartet: {task_description[:50]}"
        )
        return self._current_episode

    def end_task(
        self, success: bool, final_state: Optional[Dict[str, Any]] = None
    ) -> float:
        ...
        total_reward = self.rl_memory.end_episode(
            self._current_episode.id, success, final_state
        )

        logger.info(
            f"RL Episode {self._current_episode.id} beendet: success={success}, reward={total_reward:.2f}"
        )
    ...
    def suggest_action(
        self, state: Dict[str, Any], available_actions: List[Dict[str, Any]], goal: str
        ...
        if q_entries:
            ...
            if best_entry.confidence >= self.min_q_confidence:
                logger.info(
                    f"RL: Using Q-Table action: {best_entry.action_key} (Q={best_entry.q_value:.2f})"
                )

                # Parse action from key
                action_type, action_detail = self._parse_action_key(
                    best_entry.action_key
                )
        ...
        if available_actions:
            ...
            logger.info(
                f"RL: No Q-Table entry, using LLM plan: {action.get('text', '')[:50]}"
            )
            return ActionSuggestion(
                action_type=action.get("type", "unknown"),
                action_params=action.get("params", {}),
    ...
    def get_action_ranking(
        ...
        for action in actions:
            action_key = RLMemory.action_key(
                action.get("type", "unknown"), action.get("params", {})
            )
    ...
    def record_step(
        ...
        logger.debug(
            f"RL Step {self._step_index}: {action_text[:30]} → reward={reward:.2f}"
        )
    ...
    def apply_human_feedback(
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT reward FROM rl_transitions WHERE id = ?", (transition_id,)
        )
        row = cursor.fetchone()
        ...
        logger.info(
            f"Applied human feedback to transition {transition_id}: {feedback_type}"
        )
            ...
            entries.append(
                {
                    ...
                    "state_description": (
                        row["state_description"][:50]
                        if row["state_description"]
                        else ""
                    ),
                    "action_key": row["action_key"],
                    "action_description": (
                        row["action_description"][:50]
                        if row["action_description"]
                        else ""
                    ),
        ...
        try:
            ...
            if self._use_rl:
                ...
                total_reward = self.rl_agent.end_task(
                    success=result.get("success", False), final_state=final_state
                )
