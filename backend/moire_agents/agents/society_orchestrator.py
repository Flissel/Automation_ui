...
try:
    ...
    logging.warning(
        "autogen-agentchat not installed. Run: pip install autogen-agentchat autogen-ext[openai]"
    )
...
try:
    from memory.sqlite_memory import (ActionPattern, AgentMemory,
                                      ConversationMessage, TaskRecord,
                                      TaskStatus, UIElementCache, get_memory,
...
class DesktopTools:
            ...
            if hasattr(self.moire_client, "capture_and_wait_for_complete"):
                ...
                if result.success:
                    ...
                    if result.ui_context:
                        elements = (
                            result.ui_context.elements
                            if hasattr(result.ui_context, "elements")
                            else []
                        )
                        texts_list = [
                            {
    ...
    async def find_elements_by_category(self, category: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            or (b.get("type", "").lower() == category.lower() and not b.get("category"))
        ]

    async def click_by_category(
        self, category: str, text: Optional[str] = None, index: int = 0
    ) -> str:
        """
        ...
        if text:
            filtered = [
                e for e in elements if text.lower() in (e.get("text") or "").lower()
            ]
            if filtered:
                elements = filtered
            else:
    ...
    async def click_by_text(self, text: str, exact_match: bool = False) -> str:
        ...
        for b in boxes:
            ...
            if exact_match:
                ...
                if (
                    b_text and text.lower() in b_text.lower()
                ):  # FIX G2: Check b_text exists
                    x = b.get("x", 0) + b.get("width", 0) // 2
                    y = b.get("y", 0) + b.get("height", 0) // 2
                    return await self.click(x, y)
    ...
    async def hotkey(self, *keys: str) -> str:
        ...
        if self.moire_client and hasattr(self.moire_client, "send_command"):
            try:
                await self.moire_client.send_command(
                    {"type": "hotkey", "keys": list(keys)}
                )
                return f"Pressed hotkey: {'+'.join(keys)}"
            except Exception as e:
    ...
    async def scroll(self, direction: str = "down", amount: int = 3) -> str:
        ...
        if self.interaction_agent:
            try:
                result = await self.interaction_agent.scroll(
...
class SocietyOfMindOrchestrator:
        ...
        try:
            ...
            for pattern in hotkey_patterns:
                ...
                if match:
                    ...
                    normalized_keys = [
                        KEY_ALIASES.get(k.strip(), k.strip()) for k in keys
                    ]
            ...
            scroll_match = re.search(
                r"scroll\s+(up|down|left|right)(?:\s+(\d+))?", step_lower
            )
            if scroll_match:
            ...
            if "click" in step_lower:
                ...
                if "on" in step_lower or "the" in step_lower:
                    ...
                    if len(parts) > 1:
                        ...
                        target = re.sub(
                            r"\b(button|link|icon|menu|element)\b", "", target
                        ).strip()
                        if target:
            ...
            if "press" in step_lower:
                key_match = re.search(
                    r"press\s+(?:the\s+)?(\w+)(?:\s+key)?", step_lower
                )
                if key_match:
    ...
    async def _plan_task(
        self, goal: str, screen_state: str, error_context: Optional[str] = None
    ) -> List[str]:
        """Plant die Task."""
        try:
            prompt = f"""Goal: {goal}
...
"""
    ...
    async def _validate_step(
        self, step: str, action_result: str, screen_before: str, screen_after: str
    ) -> Dict[str, Any]:
...
CONFIDENCE: [0-140]"""
            ...
            is_error = any(
                err in action_result.lower() for err in ["error", "could not", "failed"]
            )
            return {
                "verified": not is_error,
                "changes": "Could not analyze",
    ...
    def _get_execution_summary(self, records: List[ExecutionRecord], goal: str) -> str:
        ...
        successful = [
            r for r in records if r.success and r.verification.get("verified")
        ]
        failed = [
            r for r in records if not r.success or not r.verification.get("verified")
        ]
        ...
        try:
            ...
            if execution_records:
                successful_steps = [
                    ...
                    if r.success and r.verification.get("verified")
                ]
...
- reason: explanation"""
            ...
            if response and response.content:
                ...
                if execution_records:
                    all_verified = all(
                        r.success and r.verification.get("verified")
                        for r in execution_records
                    )
                    if all_verified and not achieved:
                        # Check if last step was a "send" action (enter, click send button)
                        last_step = (
                            execution_records[-1].step_text.lower()
                            if execution_records
                            else ""
                        )
                        if any(
                            kw in last_step
                            for kw in ["enter", "send", "submit", "confirm"]
                        ):
                            achieved = True
                            logger.info(
                                "G3 FIX: All steps verified + final send action → marking as achieved"
                            )
        ...
        try:
            ...
            while iteration < max_rounds:
                ...
                if step_index >= len(plan):
                    ...
                    screen_state = await self.tools.get_screen_state()
                    goal_check = await self._check_goal(
                        goal, screen_state, execution_records
                    )

                    if goal_check.get("achieved"):
                        logger.info(
                            f"Goal achieved with confidence {goal_check.get('confidence', 0)}%"
                        )
                        ...
                        return {
                            ...
                            "rounds": [
                                r.to_summary() for r in execution_records
                            ],  # FIX G1: Add rounds key
                            "execution_records": [
                                r.to_summary() for r in execution_records
                            ],
                    ...
                    if replan_count < max_replans:
                        ...
                        execution_summary = self._get_execution_summary(
                            execution_records, goal
                        )
                        error_context = f"{execution_summary}\n\nPROBLEM: All steps executed but goal not achieved."
                        new_plan = await self._plan_task(
                            goal, screen_state, error_context
                        )
                        if new_plan:
                ...
                # Validate step
                verification = await self._validate_step(
                    current_step, result, screen_before, screen_after
                )
                ...
                if "error" in result.lower() and not verification.get(
                    "verified", False
                ):
                    ...
                    if replan_count < max_replans:
                        ...
                        new_plan = await self._plan_task(
                            goal, screen_after, error_context
                        )
                        if new_plan:
                            plan = [s for s in new_plan if s is not None and s.strip()]
                            self._current_task.plan = plan
...
async def create_society_orchestrator(
    ...
    return SocietyOfMindOrchestrator(
        moire_client=moire_client, interaction_agent=interaction_agent, model=model
    )
...
def get_society_orchestrator(
    ...
    if _society_orchestrator_instance is None:
        ...
        logger.info(
            f"SocietyOfMindOrchestrator Singleton erstellt mit Model: {model_name}"
        )
    else:
