...
from core.event_queue import (ActionEvent, ActionStatus, EventQueue, TaskEvent,
                              TaskStatus, ValidationEvent, get_event_queue)
...
try:
    from context import (ContextTracker, SelectionManager, WordHelper,
...
class OrchestratorV2:
    ...
    def _on_action_completed(self, action: ActionEvent, validation: ValidationEvent):
        ...
        logger.debug(
            f"Aktion abgeschlossen: {action.id} - Erfolg: {validation.success}"
        )
    ...
    async def execute_task(
        ...
        result = await self.execute_task_with_reflection(goal=goal, context=context)
    ...
    def set_initial_state(self, screenshot: bytes, state: Dict[str, Any]):
        ...
        logger.info(
            f"Initial state gespeichert: {len(screenshot)} bytes, "
    ...
    async def _handle_task_planning(self, task: TaskEvent) -> List[ActionEvent]:
        ...
        actions = await self.reasoning.plan_task(task, screen_state, screenshot_bytes)
    ...
    async def _execute_action(self, action: ActionEvent) -> Dict[str, Any]:
        ...
        try:
            if action_type == "click":
                ...
                return await self._interaction_agent.drag(
                ...
                return {
                    "success": False,
                    "error": f"Unbekannter Aktionstyp: {action_type}",
                }
    ...
    async def _handle_validation(
        self, action: ActionEvent, result: Dict[str, Any]
    ) -> ValidationEvent:
        """Validiert Ergebnis und analysiert State."""
        await asyncio.sleep(0.3)

        validation_result = await self.validator.validate_action(
            action, timeout=5.0, require_change=self._action_requires_change(action)
        )

        validation = await self.validator.create_validation_event(
    ...
    async def _analyze_state_after_action(
        self, action: ActionEvent, validation: ValidationEvent
    ):
        ...
        if self._current_task and not self._goal_achieved:
            goal_check = await self._check_goal_achieved(
                self._current_task.goal, new_state, new_screenshot
            )
            ...
            if goal_check["achieved"]:
                ...
                if self._iteration_count >= self._max_iterations:
                    logger.warning(
                        f"⚠ Max Iterationen ({self._max_iterations}) erreicht!"
                    )

    async def _check_goal_achieved(
        self, goal: str, state: Dict[str, Any], screenshot: Optional[bytes]
    ) -> Dict[str, Any]:
        ...
        if "öffne" in goal_lower or "open" in goal_lower:
            if state.get("elements"):
                return {
                    "achieved": True,
                    "reason": "App wurde geöffnet (Elemente erkannt)",
                }
        ...
        if self.vision_agent and self.vision_agent.is_available() and screenshot:
            try:
                ...
                if vision_check["confidence"] > 0.7:
                    return {
                        "achieved": vision_check["achieved"],
                        "reason": vision_check["reason"],
                    }
            except Exception as e:
    ...
    async def _vision_goal_check(self, goal: str, screenshot: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.vision_agent, "analyze_for_reflection"):
                ...
                logger.info(
                    f"Vision Goal Check: achieved={goal_achieved}, confidence={confidence:.0f}%"
                )
            ...
            if hasattr(self.vision_agent, "analyze_screen"):
                ...
                if analysis:
                    ...
                    confidence = (
                        (matches / len(goal_keywords)) * 100 if goal_keywords else 0
                    )
            ...
            return {
                "achieved": False,
                "confidence": 0,
                "reason": "Vision analysis not available",
            }
    ...
    async def _execute_word_format_action(self, action: ActionEvent) -> Dict[str, Any]:
        ...
        try:
            if (
                action_type == "select_paragraph"
                or "absatz" in action.description.lower()
            ):
                result = await self.word_helper.select_paragraph()
                return {"success": result, "action": "select_paragraph"}

            elif (
                action_type == "select_word"
                or "wort markier" in action.description.lower()
            ):
                result = await self.word_helper.select_word()
                return {"success": result, "action": "select_word"}

            elif (
                action_type == "format_bold"
                or "fett" in action.description.lower()
                or "bold" in action.description.lower()
            ):
                result = await self.word_helper.bold()
                return {"success": result, "action": "format_bold"}

            elif (
                action_type == "format_italic"
                or "kursiv" in action.description.lower()
                or "italic" in action.description.lower()
            ):
                result = await self.word_helper.italic()
                return {"success": result, "action": "format_italic"}

            elif (
                action_type == "format_underline"
                or "unterstreich" in action.description.lower()
            ):
                result = await self.word_helper.underline()
                return {"success": result, "action": "format_underline"}

            elif (
                action_type == "format_paragraph"
                or "absatz fett" in action.description.lower()
            ):
    ...
    async def _capture_screenshot(self) -> Optional[bytes]:
        ...
        try:
            ...
            if hasattr(self.moire_client, "capture_and_wait_for_complete"):
                result = await self.moire_client.capture_and_wait_for_complete(
                    timeout=10.0
                )
    ...
    async def check_connection_health(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            "vision_available": self.vision_agent is not None
            and (self.vision_agent.is_available() if self.vision_agent else False),
        ...
        if self.moire_client:
            try:
                ...
                if hasattr(self.moire_client, "get_last_state_timestamp"):
                    health["last_state_timestamp"] = (
                        self.moire_client.get_last_state_timestamp()
                    )
            except Exception as e:
    ...
    async def execute_task_with_reflection(
        ...
        logger.info(
            f"   Max Runden: {max_reflection_rounds}, Aktionen/Runde: {actions_per_round}"
        )
        ...
        for round_num in range(1, max_reflection_rounds + 1):
            ...
            round_context = {
                **(context or {}),
                ...
                "reflection_feedback": (
                    reflection_results[-1].to_dict() if reflection_results else None
                ),
            }
            ...
            try:
                ...
                for action_num in range(actions_per_round):
                    try:
                        completed = await self.queue.wait_for_task(
                            task.id, timeout=15.0
                        )
                        ...
                        logger.warning(
                            f"Timeout bei Aktion {action_num + 1} in Runde {round_num}"
                        )
                        continue
                    except Exception as e:
                ...
                logger.info(
                    f"   Ausgeführte Aktionen in Runde {round_num}: {len(round_actions)}"
                )
            ...
            if reflection_screenshot:
                reflection_screenshot_b64 = base64.b64encode(
                    reflection_screenshot
                ).decode("utf-8")
                logger.info(
                    f"📸 Reflection-Screenshot erfasst: {len(reflection_screenshot)} bytes"
                )
            else:
            ...
            if reflection_result.status == ReflectionStatus.NEEDS_CORRECTION:
                ...
                if replan_result.get("abort"):
                    logger.warning(
                        f"⚠ Orchestrator empfiehlt Abbruch: {replan_result.get('reason')}"
                    )
        ...
        if final_status == ReflectionStatus.IN_PROGRESS:
            ...
            logger.warning(
                f"⚠ Max Reflection-Runden ({max_reflection_rounds}) erreicht ohne Zielerreichung"
            )
    ...
    async def _perform_reflection(self, request: ReflectionRequest) -> ReflectionResult:
        ...
        vision_available = (
            self.vision_agent is not None
            and self.vision_agent.is_available()
            and screenshot is not None
        )

        logger.info(f"   Vision Agent verfügbar: {self.vision_agent is not None}")
        logger.info(
            f"   Vision Agent is_available(): {self.vision_agent.is_available() if self.vision_agent else False}"
        )
        ...
        if vision_available:
            try:
                ...
                goal_check = await self._check_goal_achieved(
                    request.original_goal, current_state, screenshot
                )
    ...
    async def _orchestrator_replan(
        self, goal: str, reflection: ReflectionResult, executed_actions: List[str]
    ) -> Dict[str, Any]:
        """
...
"""
        ...
        try:
            ...
            new_actions = await self.reasoning.plan_task(
                replan_task, screen_state, screenshot
            )
            ...
            logger.info(
                f"   Re-Planning ergab {len(priority_actions)} neue Prioritäts-Aktionen"
            )
    ...
    async def execute_task_batched(
        ...
        logger.info(
            f"   Batch-Größe: {batch_size}, Checkpoint-Intervall: {checkpoint_interval}"
        )
        logger.info(f"{'='*60}\n")
        ...
        try:
            ...
            if self.vision_agent and self.vision_agent.is_available():
                ...
                try:
                    ...
                    vision_context = (
                        vision_result if isinstance(vision_result, dict) else {}
                    )
                except Exception as e:
            ...
            for batch_idx, batch in enumerate(batches):
                ...
                batch_result = await self._wait_for_batch_completion(
                    batch, timeout=60.0
                )
                executed_actions.extend(batch)

                logger.info(
                    f"   ✓ Batch {batch_idx + 1} abgeschlossen: {batch_result['completed']}/{len(batch)} erfolgreich"
                )

                # Checkpoint-Validierung
                if (batch_idx + 1) % checkpoint_interval == 0 or batch_idx == len(
                    batches
                ) - 1:
                    logger.info(f"   🔍 Checkpoint-Validierung...")
                    checkpoint_result = await self._validate_checkpoint(
                        goal, executed_actions
                    )
                    ...
                    if checkpoint_result.get("needs_replan"):
                        ...
                        if remaining_actions:
                            ...
                            batches = self._create_action_batches(
                                remaining_actions, batch_size
                            )
            ...
            logger.info("\n📊 Finale Validierung...")
            final_screenshot = await self._capture_screenshot()
            final_result = await self._validate_final_result(
                goal, final_screenshot, executed_actions
            )
            ...
            logger.info(
                f"   Actions: {len(executed_actions)}, Batches: {total_batches}"
            )
    ...
    def _create_action_batches(
        self, actions: List[ActionEvent], batch_size: int
    ...
    async def _wait_for_batch_completion(
        self, batch: List[ActionEvent], timeout: float = 60.0
    ) -> Dict[str, Any]:
        ...
        while time.time() - start_time < timeout:
            ...
            completed = sum(
                ...
                for a in batch
                if a.status
                in [ActionStatus.COMPLETED, ActionStatus.FAILED, ActionStatus.SKIPPED]
            )

            if completed >= len(batch):
                return {
                    "completed": sum(
                        1 for a in batch if a.status == ActionStatus.COMPLETED
                    ),
                    "failed": sum(1 for a in batch if a.status == ActionStatus.FAILED),
                    "skipped": sum(
                        1 for a in batch if a.status == ActionStatus.SKIPPED
                    ),
                }
    ...
    async def _validate_checkpoint(
        self, goal: str, executed_actions: List[ActionEvent]
    ) -> Dict[str, Any]:
...
{{"goal_achieved": true/false, "needs_replan": true/false, "issues": [], "next_steps": []}}""",
                ...
                return (
                    ...
                    else {"needs_replan": False, "goal_achieved": False}
                )
            except Exception as e:
    ...
    async def _validate_final_result(
        ...
        success_count = sum(
            1 for a in executed_actions if a.status == ActionStatus.COMPLETED
        )
...
Antworte mit {{"success": true/false, "confidence": 0.0-1.0, "reason": "..."}}""",
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "has_vision_agent": self.vision_agent is not None
            and self.vision_agent.is_available(),
        ...
        if self.context_tracker:
            status["context"] = {
                "selection_active": self.context_tracker.selection.is_active,
                "selection_text": (
                    self.context_tracker.selection.text[:100]
                    if self.context_tracker.selection.text
                    else ""
                ),
                "app_type": self.context_tracker.app.app_type.value,
                "window_title": self.context_tracker.app.window_title,
